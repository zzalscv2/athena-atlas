...
namespace MuonCalib {
    ...
    CalibDbConnection::CalibDbConnection(const std::string& ConnectionString, const std::string& WorkingSchema) :
        m_connection_string(ConnectionString),
        m_working_schema(WorkingSchema),
        m_comp_loaded(false),
    ...
    coral::IRelationalDomain& CalibDbConnection::domain(const std::string& connectionString) {
        ...
        if (m_target_user != "" && m_target_user != m_username) { m_username = m_username + "[" + m_target_user + "]"; }
