...
MuonMM_CablingAlg::MuonMM_CablingAlg(const std::string& name, ISvcLocator* pSvcLocator) : AthAlgorithm(name, pSvcLocator) {}

StatusCode MuonMM_CablingAlg::initialize() {
    ATH_MSG_DEBUG("initialize " << name());
    ATH_CHECK(m_readCablingKey.initialize(m_JSONFile.value().empty() && !m_readCablingKey.empty())); // do not initialize if either an external json file is used to read the cabling map from or no folder to read the cabling from is specified 
    ATH_CHECK(m_muonManagerKey.initialize(!m_JSONFile.value().empty() || m_readCablingKey.empty()));  // only intialize if either a database folder to read the cabling  from or an external json file was defined
...
StatusCode MuonMM_CablingAlg::execute() {
    ...
    if (!m_readCablingKey.empty()) {
        SG::ReadCondHandle<CondAttrListCollection> condHandle{m_readCablingKey, ctx};
        if (!condHandle.isValid()) {
            ATH_MSG_FATAL("Failed to retrive the cabling data from the database "<<m_readCablingKey.fullKey());
            return StatusCode::FAILURE;
        }
    }

    std::unique_ptr<MicroMega_CablingMap> writeCdo{std::make_unique<MicroMega_CablingMap>(m_idHelperSvc.get())};
    /// JSON file
...
StatusCode MuonMM_CablingAlg::loadCablingSchema(const std::string& theJSON, MicroMega_CablingMap& cabling_map) const {
    ...
    for (const auto &db_channel : payload.items()) {
        ...
        bool isValid{false};
        const Identifier gap_id = id_helper.channelID(stName,eta,phi,multilayer,gap, 1, isValid);
        if (!isValid) {
            ATH_MSG_FATAL("Failed to deduce a valid identifier from st:"<<stName<<" eta: "<<eta<<" phi: "<<phi<<" multilayer: "<<multilayer<<" gasgap: "<<gap);
            return StatusCode::FAILURE;
        }
        ...
        if (!cabling_map.addConnector(gap_id, zebra_connector, msgStream())) return StatusCode::FAILURE;
    }
    return StatusCode::SUCCESS;
}
