...
std::ostream& operator<<(std::ostream& ostr, const MdtMezzanineCard& map) {
    ...
    for (uint8_t lay = map.numTubeLayers(); lay > 0 ; --lay){
       ...
       for (uint8_t tube = 1; tube <= map.numTubesPerLayer(); ++tube) {          
          ...
          ostr<<std::setw(5)<<static_cast<int>(map.tubeToTdcMap()[tubeNum]);
       }
       ostr<<std::endl; 
    }
    return ostr;
...
MdtMezzanineCard::MdtMezzanineCard(const Mapping& tdcToTubeMap, uint8_t num_layers, uint8_t mezz_id):
    ...
    for (unsigned int tdc = 0 ; tdc < m_tdcToTubes.size() ; ++tdc) {
        ...
        if (tube < m_tubesToTdc.size()) m_tubesToTdc[tube] = tdc;
    }
}
...
bool MdtMezzanineCard::checkConsistency(MsgStream& log) const {
    ...
    if (numTubeLayers() != 3 && numTubeLayers() != 4){
        log<<MSG::ERROR<<"Mezzanine card "<<static_cast<int>(id())
                       <<" has invalid many tube layers "<<static_cast<int>(numTubeLayers())<<endmsg;
        return false;
    }
    ...
    unsigned int unmapped = std::count_if(m_tdcToTubes.begin(), m_tdcToTubes.end(), 
                                    [](const uint8_t ch){ return ch == NOTSET;});
    if ((uniqueSet.size() + unmapped) != m_tdcToTubes.size()) {
        log<<MSG::ERROR<<"Mezzanine card has unassigned tdc -> tube channels "<<endmsg;
        log<<MSG::ERROR<<"Mapped channels "<<uniqueSet.size()<<" dead channels: "<<unmapped<<std::endl;
        log<<MSG::ERROR<<"Please check "<<(*this)<<endmsg;
        return false;
    }
    if (unmapped == m_tdcToTubes.size()) {
        log<<MSG::ERROR<<"Mezzanine card does not have any associated channel "<<endmsg;
        log<<MSG::ERROR<<"Please check "<<(*this)<<endmsg;
    }
    uniqueSet.clear();
    ...
    if ((uniqueSet.size() + unmapped) != m_tubesToTdc.size()) {
        log<<MSG::ERROR<<" Mezzanine card maps tubes -> tdc inconsitently "<<endmsg;
        log<<MSG::ERROR<<"Mapped channels "<<uniqueSet.size()<<" dead channels: "<<unmapped<<std::endl;
        log<<MSG::ERROR<<" Please check "<<(*this)<<endmsg;
        return false;
    }
    if (debug) {
        log<<MSG::VERBOSE<<" All checks passed "<<std::endl<<(*this)<<endmsg;
    }    
    return true;
}
