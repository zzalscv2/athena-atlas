...
public:
    ...
    // The following function corrects the MM cabling. It takes an identifier as input (nominal identifer from the decoder) and returns
    ...
    //      A nullopt if the correction would move a channel outside the defined range of channel to be shifted (e.g. the first channel of a zebra connector which would be shifted into the channel range of the previous connector wich is physically impossible.) 
    ...
    bool addConnector(const Identifier& gapID, const MicroMegaZebraData& connector, MsgStream& msg );
...
private:
    ...
    // Map holding the MM cabling correction map. The key is the identifier of the gas gap and the value is a  MicroMegaZebraSet object containing the information on which channel range needs to be moved
