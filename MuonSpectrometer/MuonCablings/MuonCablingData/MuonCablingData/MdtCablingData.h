...
struct MdtCablingData : public MdtCablingOffData, MdtCablingOnData {
    ...
    uint8_t channelId{
        0xff};  /// Identifier of the corresponding channel on the tdc

    /// In the Muon spectrometer layouts, before Run 2, no chamber had more than
    /// 100 tubes per multilayer. Hence, the the tube layer and multilayer
    /// information is encoded into a 4 digit long number
    ///           MLTT
    /// where M represents the multilayer number, L the layer number and TT is
    /// the tube numbering per each multilayer
    ...
    bool operator==(const MdtCablingData& other) const {
        return this->MdtCablingOffData::operator==(other) &&
               this->MdtCablingOnData::operator==(other) &&
               tdcId == other.tdcId && layer == other.layer &&
               tube == other.tube && channelId == other.channelId;
    }
    bool operator!=(const MdtCablingData& other) const {
        return !((*this) == other);
    }
    bool operator<(const MdtCablingData& other) const {
        if (this->MdtCablingOffData::operator!=(other))
            return this->MdtCablingOffData::operator<(other);
        if (this->MdtCablingOnData::operator!=(other))
            return this->MdtCablingOnData::operator<(other);
        if (tdcId != other.tdcId)
