    ...
    SG::WriteCondHandle<Trk::TrackingGeometry>& whandle) const
{
   ...
   ///////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    if (!aLVC.m_stationSpan)
        aLVC.m_stationSpan = findVolumesSpan(stations.get(), 100. * m_alignTolerance, m_alignTolerance * Gaudi::Units::deg, aLVC);
    ...
    if (aLVC.m_adjustStatic && aLVC.m_static3d)
        ...
        muonBarrel = processVolume(&barrelVol, m_barrelEtaPartition, m_phiPartition, "Muon::Detectors::Barrel", aLVC, hasStations);
    // inner Endcap
    ...
    if (aLVC.m_adjustStatic && aLVC.m_static3d)
        ...
        negativeMuonSmallWheel =
            processVolume(&negSWVol, m_innerEndcapEtaPartition, m_phiPartition, "Muon::Detectors::NegativeSmallWheel", aLVC, hasStations);
    ...
    if (aLVC.m_adjustStatic && aLVC.m_static3d)
        ...
        positiveMuonSmallWheel =
            processVolume(&posSWVol, m_innerEndcapEtaPartition, m_phiPartition, "Muon::Detectors::PositiveSmallWheel", aLVC, hasStations);
    ...
    if (aLVC.m_adjustStatic && aLVC.m_static3d)
        ...
        negativeECT =
            processVolume(&negECTVol, m_innerEndcapEtaPartition, m_phiPartition, "Muon::Detectors::NegativeECT", aLVC, hasStations);
    ...
    if (aLVC.m_adjustStatic && m_static3d)
        ...
        positiveECT =
            processVolume(&posECTVol, m_innerEndcapEtaPartition, m_phiPartition, "Muon::Detectors::PositiveECT", aLVC, hasStations);
    ...
    if (aLVC.m_adjustStatic && aLVC.m_static3d)
        ...
        negativeMuonOuterWheel =
            processVolume(&negOWVol, m_outerEndcapEtaPartition, m_phiPartition, "Muon::Detectors::NegativeOuterWheel", aLVC, hasStations);
    //
    ...
    if (aLVC.m_adjustStatic && aLVC.m_static3d)
        ...
        positiveMuonOuterWheel =
            processVolume(&posOWVol, m_outerEndcapEtaPartition, m_phiPartition, "Muon::Detectors::PositiveOuterWheel", aLVC, hasStations);
    // outer buffer
    ...
    if (aLVC.m_adjustStatic && aLVC.m_static3d)
        ...
        negativeMuonBigWheel =
            processVolume(&negBWVol, m_outerEndcapEtaPartition, m_phiPartition, "Muon::Detectors::NegativeBigWheel", aLVC, hasStations);
    //
    ...
    if (aLVC.m_adjustStatic && aLVC.m_static3d)
        ...
        positiveMuonBigWheel =
            processVolume(&posBWVol, m_outerEndcapEtaPartition, m_phiPartition, "Muon::Detectors::PositiveBigWheel", aLVC, hasStations);
    // glue
    ...
    trackingGeometry->m_muonStations = std::move(stations);
    trackingGeometry->m_muonInertObjs = std::move(inertObjs);
...
Muon::MuonTrackingGeometryBuilderCond::findVolumesSpan(const std::vector<std::unique_ptr<Trk::DetachedTrackingVolume> >* objs,
                                                       double zTol, double phiTol, LocalVariablesContainer& aLVC) const {
...
Trk::TrackingVolume* Muon::MuonTrackingGeometryBuilderCond::processVolume(const Trk::Volume* vol, int etaN, int phiN,
    ...
    if (etaN * phiN > 1) {  // partition
        ...
        if (hasStations) { muonObjs = getDetachedObjects(vol, blendVols, aLVC); }
...
Trk::TrackingVolume* Muon::MuonTrackingGeometryBuilderCond::processVolume(const Trk::Volume* vol, int mode, const std::string& volumeName,
        ...
        for (unsigned int eta = 0; eta < zSteps.size() - 1; eta++) {
            ...
            for (unsigned int phi = 0; phi < phiN; phi++) {
                ...
                for (unsigned int h = 0; h < hSteps.size() - 1; h++) {
                    ...
                    subVolumesVect.emplace_back(Trk::SharedObject<Trk::TrackingVolume>(sVol),
                                                                              Amg::Vector3D((*transf) * gp));
        ...
        for (unsigned int eta = 0; eta < zSteps.size() - 1; eta++) {
            ...
            for (unsigned int phi = 0; phi < phiN; phi++) {
                ...
                subVolumes[phi * etaN + eta] =
                    Trk::TrackingVolumeOrderPosition(Trk::SharedObject<Trk::TrackingVolume>(sVol), Amg::Vector3D((*transf) * gp));
                // glue subVolumes
...
Trk::TrackingVolume* Muon::MuonTrackingGeometryBuilderCond::processShield(const Trk::Volume* vol, int type, const std::string& volumeName,
                                                                                LocalVariablesContainer& aLVC, bool hasStations) const {
...
std::vector<Trk::DetachedTrackingVolume*>* Muon::MuonTrackingGeometryBuilderCond::getDetachedObjects(
...
void Muon::MuonTrackingGeometryBuilderCond::checkVolume(Trk::TrackingVolume* vol) {
