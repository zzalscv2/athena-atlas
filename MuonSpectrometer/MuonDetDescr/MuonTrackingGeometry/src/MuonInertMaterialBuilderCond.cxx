...
// constructor
Muon::MuonInertMaterialBuilderCond::MuonInertMaterialBuilderCond(const std::string& t, const std::string& n, const IInterface* p) :
    ...
    for (; msTypeIter != msTypes->end(); ++msTypeIter) {
        ...
        if (blend) {
            ...
            for (const auto & ic : *(*msTypeIter).first->constituents()) {
                protMass += calculateVolume(ic.first.get()) *
                            ic.second;
            }
...
Muon::MuonInertMaterialBuilderCond::buildDetachedTrackingVolumeTypes(const EventContext& ctx,
    ...
    while (!vol.atEnd()) {
        ...
        if (vname.size() > 7 && vname.substr(vname.size() - 7, 7) == "Station") {  // do nothing, active station
            ...
            // Reserving vols.size()+3 should be enough. Without reserving the vectors in the Volumes point to invalid memory,
            // as otherwise the outer vector 'constituentsVector' will be resized and reallocated
    ...
    int count = 0;
    for (auto & mObject : *mObjects) count += mObject.second.size();
...
void Muon::MuonInertMaterialBuilderCond::printInfo(const GeoVPhysVol* pv) const {
...
void Muon::MuonInertMaterialBuilderCond::printChildren(const GeoVPhysVol* pv) const {
    // subcomponents
    ...
    if (simpleMode == 1) {
        ...
        if (constituents.size() == 1) {  // simplified volume : the scale factor refers to the density scaling
            ...
            for (auto & constituent : constituents) {
                ...
                // simplified material rescales X0, l0 and density
                Trk::Material mat(trVol->X0 / fraction, trVol->L0 / fraction, trVol->A, trVol->Z, fraction * trVol->rho);
                Trk::TrackingVolume* trc = new Trk::TrackingVolume(*(constituent.first), mat, nullptr, nullptr, trVol->volumeName());
    ...
    if (blend) {
        ...
        for (auto & constituent : constituents) {
            ...
            confinedConst.emplace_back(
                std::make_unique<Trk::Volume>(*(constituent.first), newVol->transform().inverse()), scale);
        }
...
double Muon::MuonInertMaterialBuilderCond::calculateVolume(const Trk::Volume* envelope) const {
    double envVol = 0.;
...
Amg::Vector3D Muon::MuonInertMaterialBuilderCond::getScanPoint(const Trk::Volume* vol) const {
...
std::vector<std::pair<const Trk::Volume*, std::pair<float, float>>> Muon::MuonInertMaterialBuilderCond::splitComposedVolume(
