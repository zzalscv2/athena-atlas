...
Muon::MuonTrackingGeometryBuilder::findVolumesSpan(const std::vector<Trk::DetachedTrackingVolume*>* objs, double zTol, double phiTol,
    ...
    for (auto *obj : *objs) {
        ...
        if ((*span)[0] < -m_bigWheel) {
            (*spans)[0]->push_back(std::pair<Trk::DetachedTrackingVolume*, const Span*>(obj, span));
            nspans++;
        }
        // negative big wheel
        if ((*span)[0] < -aLVC.m_innerEndcapZ && (*span)[1] > -m_bigWheel) {
            (*spans)[1]->push_back(std::pair<Trk::DetachedTrackingVolume*, const Span*>(obj, span));
            nspans++;
        }
        // neg.ect
        if ((*span)[0] < -m_ectZ && (*span)[1] > -aLVC.m_innerEndcapZ) {
            (*spans)[2]->push_back(std::pair<Trk::DetachedTrackingVolume*, const Span*>(obj, span));
            nspans++;
        }
        // neg.small wheel
        if ((*span)[0] < -m_diskShieldZ && (*span)[1] > -m_ectZ) {
            (*spans)[3]->push_back(std::pair<Trk::DetachedTrackingVolume*, const Span*>(obj, span));
            nspans++;
        }
        // barrel
        if ((*span)[0] < m_diskShieldZ && (*span)[1] > -m_diskShieldZ) {
            //	 && ((*span)[5]> m_innerBarrelRadius || (*span)[0]<-m_barrelZ || (*span)[1]>m_barrelZ)  ) {
            (*spans)[4]->push_back(std::pair<Trk::DetachedTrackingVolume*, const Span*>(obj, span));
            nspans++;
        }
        // pos.small wheel
        if ((*span)[0] < m_ectZ && (*span)[1] > m_diskShieldZ) {
            (*spans)[5]->push_back(std::pair<Trk::DetachedTrackingVolume*, const Span*>(obj, span));
            nspans++;
        }
        // pos.ect
        if ((*span)[0] < aLVC.m_innerEndcapZ && (*span)[1] > m_ectZ) {
            (*spans)[6]->push_back(std::pair<Trk::DetachedTrackingVolume*, const Span*>(obj, span));
            nspans++;
        }
        // positive big wheel
        if ((*span)[0] < m_bigWheel && (*span)[1] > aLVC.m_innerEndcapZ) {
            (*spans)[7]->push_back(std::pair<Trk::DetachedTrackingVolume*, const Span*>(obj, span));
            nspans++;
        }
        // positive outer wheel
        if ((*span)[1] > m_bigWheel) {
            (*spans)[8]->push_back(std::pair<Trk::DetachedTrackingVolume*, const Span*>(obj, span));
            nspans++;
        }
