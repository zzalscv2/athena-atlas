...
StatusCode Muon::MuonTrackingGeometryBuilder::initialize() {
    ...
    if (m_chronoStatSvc.retrieve().isFailure()) { ATH_MSG_WARNING("Could not retrieve Tool " << m_chronoStatSvc << ". Exiting."); }
...
Trk::TrackingGeometry* Muon::MuonTrackingGeometryBuilder::trackingGeometry(Trk::TrackingVolume* tvol) const {
    ...
    if (m_inertObjs && m_blendInertMaterial && m_removeBlended) {
        while (m_inertPerm < m_inertObjs->size() &&
               (*m_inertObjs)[m_inertPerm]->name().substr((*m_inertObjs)[m_inertPerm]->name().size() - 4, 4) == "PERM")
            m_inertPerm++;
    }

    // find object's span with tolerance for the alignment
    if (!aLVC.m_stationSpan)
        aLVC.m_stationSpan = findVolumesSpan(m_stations, 100. * m_alignTolerance, m_alignTolerance * Gaudi::Units::deg, aLVC);
    ...
    if (m_enclosingEnvelopeSvc) {
        ...
        for (unsigned int i = 0; i < envelopeDefsIn.size(); i++) {
            if (envelopeDefsIn[i].second > envelopeDefsIn[ii].second)
                ii = i;
            else if (envelopeDefsIn[i].second == envelopeDefsIn[ii].second && envelopeDefsIn[i].first > envelopeDefsIn[ii].first)
                ii = i;
        }
    ...
    if (tvol) {
        ...
        if (enclosedDetectorHalfZ > m_barrelZ) {
            ...
            if (enclosedDetectorHalfZ < m_barrelZ) {
                ...
                Trk::TrackingVolume* barrelZPBuffer =
                    new Trk::TrackingVolume(new Amg::Transform3D(Amg::Translation3D(Amg::Vector3D(0., 0., zbShift))), barrelZPBounds,
                                            aLVC.m_muonMaterial, dummyLayers, dummyVolumes, "BarrelRZPosBuffer");
                Trk::TrackingVolume* barrelZMBuffer =
                    new Trk::TrackingVolume(new Amg::Transform3D(Amg::Translation3D(Amg::Vector3D(0., 0., -zbShift))), barrelZMBounds,
            ...
            if (!envelopeDefs.empty()) {
                if (rmax > 0. && rmax <= aLVC.m_innerBarrelRadius && zmax > 0. && zmax <= m_barrelZ) {
                    ...
                    ATH_MSG_INFO(name() << " input MSEntrance size (R,Z:" << rmax << "," << zmax
                                        << ") clashes with MS material, switch to default values (R,Z:" << aLVC.m_innerBarrelRadius << ","
                                        << m_barrelZ << ")");
                }
            }
        }
    ...
    if (aLVC.m_msCutoutsIn[5].second != aLVC.m_innerEndcapZ) { aLVC.m_innerEndcapZ = aLVC.m_msCutoutsIn[5].second; }
    ...
    Trk::TrackingVolume* central = m_trackingVolumeHelper->glueTrackingVolumeArrays(*centralP, Trk::negativeFaceXY, *negDiskShield,
                                                                                          Trk::positiveFaceXY, "Muon::Container::Central");
    ...
    // barrel
    if (aLVC.m_adjustStatic && aLVC.m_static3d)
        muonBarrel = processVolume(&barrelVol, 0, "Muon::Detectors::Barrel", aLVC);
    else if (aLVC.m_adjustStatic)
        muonBarrel = processVolume(&barrelVol, -1, "Muon::Detectors::Barrel", aLVC);
    else
        muonBarrel = processVolume(&barrelVol, m_barrelEtaPartition, m_phiPartition, "Muon::Detectors::Barrel", aLVC);
    // inner Endcap
    // build as smallWheel+ECT
    // small wheel
    double smallWheelZHalfSize = 0.5 * (m_ectZ - m_diskShieldZ);
    negativeSmallWheelBounds = new Trk::CylinderVolumeBounds(m_innerShieldRadius, aLVC.m_outerBarrelRadius, smallWheelZHalfSize);
    ...
    if (aLVC.m_adjustStatic && aLVC.m_static3d)
        negativeMuonSmallWheel = processVolume(&negSWVol, 1, "Muon::Detectors::NegativeSmallWheel", aLVC);
    else if (aLVC.m_adjustStatic)
        negativeMuonSmallWheel = processVolume(&negSWVol, -1, "Muon::Detectors::NegativeSmallWheel", aLVC);
    else
        negativeMuonSmallWheel =
            processVolume(&negSWVol, m_innerEndcapEtaPartition, m_phiPartition, "Muon::Detectors::NegativeSmallWheel", aLVC);
    ...
    if (aLVC.m_adjustStatic && aLVC.m_static3d)
        positiveMuonSmallWheel = processVolume(&posSWVol, 1, "Muon::Detectors::PositiveSmallWheel", aLVC);
    else if (aLVC.m_adjustStatic)
        positiveMuonSmallWheel = processVolume(&posSWVol, -1, "Muon::Detectors::PositiveSmallWheel", aLVC);
    else
        positiveMuonSmallWheel =
            processVolume(&posSWVol, m_innerEndcapEtaPartition, m_phiPartition, "Muon::Detectors::PositiveSmallWheel", aLVC);
    ...
    if (aLVC.m_adjustStatic && aLVC.m_static3d)
        negativeECT = processVolume(&negECTVol, 2, "Muon::Detectors::NegativeECT", aLVC);
    else if (aLVC.m_adjustStatic)
        negativeECT = processVolume(&negECTVol, -1, "Muon::Detectors::NegativeECT", aLVC);
    else
        negativeECT = processVolume(&negECTVol, m_innerEndcapEtaPartition, m_phiPartition, "Muon::Detectors::NegativeECT", aLVC);
    ...
    if (aLVC.m_adjustStatic && m_static3d)
        positiveECT = processVolume(&posECTVol, 2, "Muon::Detectors::PositiveECT", aLVC);
    else if (aLVC.m_adjustStatic)
        positiveECT = processVolume(&posECTVol, -1, "Muon::Detectors::PositiveECT", aLVC);
    else
        positiveECT = processVolume(&posECTVol, m_innerEndcapEtaPartition, m_phiPartition, "Muon::Detectors::PositiveECT", aLVC);
    // checkVolume(positiveECT);
    // glue
    Trk::TrackingVolume* negativeMuonInnerEndcap = m_trackingVolumeHelper->glueTrackingVolumeArrays(
        *negativeECT, Trk::positiveFaceXY, *negativeMuonSmallWheel, Trk::negativeFaceXY, "Muon::Container::NegInnerEndcap");
    Trk::TrackingVolume* positiveMuonInnerEndcap = m_trackingVolumeHelper->glueTrackingVolumeArrays(
        *positiveMuonSmallWheel, Trk::positiveFaceXY, *positiveECT, Trk::negativeFaceXY, "Muon::Container::PosInnerEndcap");
    ...
    negativeOuterWheelBounds = new Trk::CylinderVolumeBounds(m_outerShieldRadius, aLVC.m_outerBarrelRadius, outerWheelZHalfSize);
    ...
    if (aLVC.m_adjustStatic && aLVC.m_static3d)
        negativeMuonOuterWheel = processVolume(&negOWVol, 3, "Muon::Detectors::NegativeOuterWheel", aLVC);
    else if (aLVC.m_adjustStatic)
        negativeMuonOuterWheel = processVolume(&negOWVol, -1, "Muon::Detectors::NegativeOuterWheel", aLVC);
    else
        negativeMuonOuterWheel =
            processVolume(&negOWVol, m_outerEndcapEtaPartition, m_phiPartition, "Muon::Detectors::NegativeOuterWheel", aLVC);
    //
    ...
    if (aLVC.m_adjustStatic && aLVC.m_static3d)
        positiveMuonOuterWheel = processVolume(&posOWVol, 3, "Muon::Detectors::PositiveOuterWheel", aLVC);
    else if (aLVC.m_adjustStatic)
        positiveMuonOuterWheel = processVolume(&posOWVol, -1, "Muon::Detectors::PositiveOuterWheel", aLVC);
    else
        positiveMuonOuterWheel =
            processVolume(&posOWVol, m_outerEndcapEtaPartition, m_phiPartition, "Muon::Detectors::PositiveOuterWheel", aLVC);
    // outer buffer
    double outerBufferZHalfSize = 0.5 * (m_outerWheel - m_bigWheel);
    negativeOuterBufferBounds = new Trk::CylinderVolumeBounds(m_outerShieldRadius, aLVC.m_outerBarrelRadius, outerBufferZHalfSize);
    ...
    if (aLVC.m_adjustStatic && aLVC.m_static3d)
        negativeMuonOuterBuffer = processVolume(&negBuffVol, 3, "Muon::Detectors::NegativeOuterBuffer", aLVC);
    else if (aLVC.m_adjustStatic)
        negativeMuonOuterBuffer = processVolume(&negBuffVol, -1, "Muon::Detectors::NegativeOuterBuffer", aLVC);
    else
        negativeMuonOuterBuffer =
            processVolume(&negBuffVol, m_outerEndcapEtaPartition, m_phiPartition, "Muon::Detectors::NegativeOuterBuffer", aLVC);
    //
    ...
    if (aLVC.m_adjustStatic && aLVC.m_static3d)
        positiveMuonOuterBuffer = processVolume(&posBuffVol, 3, "Muon::Detectors::PositiveOuterBuffer", aLVC);
    else if (aLVC.m_adjustStatic)
        positiveMuonOuterBuffer = processVolume(&posBuffVol, -1, "Muon::Detectors::PositiveOuterBuffer", aLVC);
    else
        positiveMuonOuterBuffer =
            processVolume(&posBuffVol, m_outerEndcapEtaPartition, m_phiPartition, "Muon::Detectors::PositiveOuterBuffer", aLVC);
    // big wheel
    double bigWheelZHalfSize = 0.5 * (m_bigWheel - aLVC.m_innerEndcapZ);
    negativeBigWheelBounds = new Trk::CylinderVolumeBounds(m_outerShieldRadius, aLVC.m_outerBarrelRadius, bigWheelZHalfSize);
    ...
    if (aLVC.m_adjustStatic && aLVC.m_static3d)
        negativeMuonBigWheel = processVolume(&negBWVol, 3, "Muon::Detectors::NegativeBigWheel", aLVC);
    else if (aLVC.m_adjustStatic)
        negativeMuonBigWheel = processVolume(&negBWVol, -1, "Muon::Detectors::NegativeBigWheel", aLVC);
    else
        negativeMuonBigWheel =
            processVolume(&negBWVol, m_outerEndcapEtaPartition, m_phiPartition, "Muon::Detectors::NegativeBigWheel", aLVC);
    //
    ...
    if (aLVC.m_adjustStatic && aLVC.m_static3d)
        positiveMuonBigWheel = processVolume(&posBWVol, 3, "Muon::Detectors::PositiveBigWheel", aLVC);
    else if (aLVC.m_adjustStatic)
        positiveMuonBigWheel = processVolume(&posBWVol, -1, "Muon::Detectors::PositiveBigWheel", aLVC);
    else
        positiveMuonBigWheel =
            processVolume(&posBWVol, m_outerEndcapEtaPartition, m_phiPartition, "Muon::Detectors::PositiveBigWheel", aLVC);
    // glue
    Trk::TrackingVolume* negNavOEndcap = m_trackingVolumeHelper->glueTrackingVolumeArrays(
        *negativeMuonOuterWheel, Trk::positiveFaceXY, *negativeMuonOuterBuffer, Trk::negativeFaceXY, "Muon::Container::NegOEndcap");
    Trk::TrackingVolume* posNavOEndcap = m_trackingVolumeHelper->glueTrackingVolumeArrays(
        *positiveMuonOuterBuffer, Trk::positiveFaceXY, *positiveMuonOuterWheel, Trk::negativeFaceXY, "Muon::Container::PosOEndcap");
    Trk::TrackingVolume* negativeMuonOuterEndcap = m_trackingVolumeHelper->glueTrackingVolumeArrays(
        *negNavOEndcap, Trk::positiveFaceXY, *negativeMuonBigWheel, Trk::negativeFaceXY, "Muon::Container::NegOuterEndcap");
    Trk::TrackingVolume* positiveMuonOuterEndcap = m_trackingVolumeHelper->glueTrackingVolumeArrays(
        *positiveMuonBigWheel, Trk::positiveFaceXY, *posNavOEndcap, Trk::negativeFaceXY, "Muon::Container::PosOuterEndcap");
    ...
    Trk::TrackingVolume* barrel = m_trackingVolumeHelper->glueTrackingVolumeArrays(*muonBarrel, Trk::tubeInnerCover, *central,
                                                                                         Trk::cylinderCover, "All::Container::Barrel");
    // checkVolume(barrel);
    // shield+outerEndcap
    ATH_MSG_INFO(name() << "glue shield+outerEndcap");
    Trk::TrackingVolume* negOuterEndcap = m_trackingVolumeHelper->glueTrackingVolumeArrays(
        *negativeMuonOuterEndcap, Trk::tubeInnerCover, *negOuterShield, Trk::tubeOuterCover, "Muon::Container::NegativeOuterEndcap");
    // checkVolume(negOuterEndcap);
    Trk::TrackingVolume* posOuterEndcap = m_trackingVolumeHelper->glueTrackingVolumeArrays(
        *positiveMuonOuterEndcap, Trk::tubeInnerCover, *posOuterShield, Trk::tubeOuterCover, "Muon::Container::PositiveOuterEndcap");
    // checkVolume(posOuterEndcap);
    // shield+innerEndcap
    ATH_MSG_INFO(name() << "glue shield+innerEndcap");
    Trk::TrackingVolume* negInnerEndcap = m_trackingVolumeHelper->glueTrackingVolumeArrays(
        *negativeMuonInnerEndcap, Trk::tubeInnerCover, *negInnerShield, Trk::tubeOuterCover, "Muon::Container::NegativeInnerEndcap");
    // checkVolume(negInnerEndcap);
    Trk::TrackingVolume* posInnerEndcap = m_trackingVolumeHelper->glueTrackingVolumeArrays(
        *positiveMuonInnerEndcap, Trk::tubeInnerCover, *posInnerShield, Trk::tubeOuterCover, "Muon::Container::PositiveInnerEndcap");
    ...
    Trk::TrackingVolume* negDet = m_trackingVolumeHelper->glueTrackingVolumeArrays(*negEndcap, Trk::positiveFaceXY, *barrel,
    ...
    if (m_stations) {
        for (size_t i = 0; i < m_stations->size(); i++) {
            if ((*m_stations)[i])
                ...
                ATH_MSG_DEBUG(name() << " station pointer corrupted ! ");
        }
        delete m_stations;
        m_stations = nullptr;
    }
    if (m_inertObjs) {
        ...
        for (size_t i = 0; i < inLim; i++) {
            if ((*m_inertObjs)[i])
                ...
                ATH_MSG_DEBUG(name() << " inert object pointer corrupted ! ");
        }
        delete m_inertObjs;
        m_inertObjs = nullptr;
    }
...
const Muon::Span* Muon::MuonTrackingGeometryBuilder::findVolumeSpan(const Trk::VolumeBounds* volBounds, const Amg::Transform3D& transform,
    ...
    if (comb) {
        ...
        ATH_MSG_VERBOSE("Combined span1:" << name() << "," << (*s1)[0] << "," << (*s1)[1] << "," << (*s1)[2] << "," << (*s1)[3] << ","
                                          << (*s1)[4] << "," << (*s1)[5]);
        ATH_MSG_VERBOSE("Combined span2:" << name() << "," << (*s2)[0] << "," << (*s2)[1] << "," << (*s2)[2] << "," << (*s2)[3] << ","
                                          << (*s2)[4] << "," << (*s2)[5]);
...
#undef double
    ...
    for (unsigned int ie = 0; ie < edges.size(); ie++) {
        ...
        ATH_MSG_VERBOSE(" local edges:" << ie << " x " << edges[ie].x() << " y " << edges[ie].y() << " z " << edges[ie].z() << " phi "
                                        << edges[ie].phi());
...
Muon::MuonTrackingGeometryBuilder::findVolumesSpan(const std::vector<Trk::DetachedTrackingVolume*>* objs, double zTol, double phiTol,
    ...
    for (unsigned int iobj = 0; iobj < objs->size(); iobj++) {
        ...
        if ((*span)[0] < -m_bigWheel) {
            (*spans)[0]->push_back(std::pair<Trk::DetachedTrackingVolume*, const Span*>((*objs)[iobj], span));
            nspans++;
        }
        // negative big wheel
        if ((*span)[0] < -aLVC.m_innerEndcapZ && (*span)[1] > -m_bigWheel) {
            (*spans)[1]->push_back(std::pair<Trk::DetachedTrackingVolume*, const Span*>((*objs)[iobj], span));
            nspans++;
        }
        // neg.ect
        if ((*span)[0] < -m_ectZ && (*span)[1] > -aLVC.m_innerEndcapZ) {
            (*spans)[2]->push_back(std::pair<Trk::DetachedTrackingVolume*, const Span*>((*objs)[iobj], span));
            nspans++;
        }
        // neg.small wheel
        if ((*span)[0] < -m_diskShieldZ && (*span)[1] > -m_ectZ) {
            (*spans)[3]->push_back(std::pair<Trk::DetachedTrackingVolume*, const Span*>((*objs)[iobj], span));
            nspans++;
        }
        // barrel
        if ((*span)[0] < m_diskShieldZ && (*span)[1] > -m_diskShieldZ) {
            //	 && ((*span)[5]> m_innerBarrelRadius || (*span)[0]<-m_barrelZ || (*span)[1]>m_barrelZ)  ) {
            (*spans)[4]->push_back(std::pair<Trk::DetachedTrackingVolume*, const Span*>((*objs)[iobj], span));
            nspans++;
        }
        // pos.small wheel
        if ((*span)[0] < m_ectZ && (*span)[1] > m_diskShieldZ) {
            (*spans)[5]->push_back(std::pair<Trk::DetachedTrackingVolume*, const Span*>((*objs)[iobj], span));
            nspans++;
        }
        // pos.ect
        if ((*span)[0] < aLVC.m_innerEndcapZ && (*span)[1] > m_ectZ) {
            (*spans)[6]->push_back(std::pair<Trk::DetachedTrackingVolume*, const Span*>((*objs)[iobj], span));
            nspans++;
        }
        // positive big wheel
        if ((*span)[0] < m_bigWheel && (*span)[1] > aLVC.m_innerEndcapZ) {
            (*spans)[7]->push_back(std::pair<Trk::DetachedTrackingVolume*, const Span*>((*objs)[iobj], span));
            nspans++;
        }
        // positive outer wheel
        if ((*span)[1] > m_bigWheel) {
            (*spans)[8]->push_back(std::pair<Trk::DetachedTrackingVolume*, const Span*>((*objs)[iobj], span));
            nspans++;
        }
    ...
    if (etaN * phiN > 1) {  // partition
        ...
        for (int eta = 0; eta < etaN; eta++) {
            ...
            for (int phi = 0; phi < phiN; phi++) {
                ...
                blendVols.clear();
                std::vector<Trk::DetachedTrackingVolume*>* detVols = getDetachedObjects(subVol, blendVols, aLVC);
                ...
                subVolumes.emplace_back(
                    Trk::SharedObject<Trk::TrackingVolume>(sVol, Trk::do_not_delete<Trk::TrackingVolume>),
                    Amg::Vector3D(transf * gp));
                // glue subVolumes
        ...
        // Trk::BinUtility2DPhiZ* volBinUtil=new Trk::BinUtility2DPhiZ(phiN,etaN,subBds->outerRadius(),cyl->halflengthZ(),M_PI, new
        ...
        // enclosed muon objects ?
        blendVols.clear();
        std::vector<Trk::DetachedTrackingVolume*>* muonObjs = getDetachedObjects(vol, blendVols, aLVC);
...
Trk::TrackingVolume* Muon::MuonTrackingGeometryBuilder::processShield(const Trk::Volume* vol, int type, const std::string& volumeName,
                                                                            LocalVariablesContainer& aLVC) const {
    ...
    for (unsigned int eta = 0; eta < zSteps.size() - 1; eta++) {
        ...
        for (unsigned int h = 0; h < hSteps.size() - 1; h++) {
            ...
            Amg::Transform3D* transf =
                new Amg::Transform3D(Amg::AngleAxis3D(posPhi, Amg::Vector3D(0., 0., 1.)) * Amg::Translation3D(Amg::Vector3D(0., 0., posZ)));
            Trk::Volume subVol(transf, subBds);
...
std::vector<Trk::DetachedTrackingVolume*>* Muon::MuonTrackingGeometryBuilder::getDetachedObjects(
    const Trk::Volume* vol, std::vector<Trk::DetachedTrackingVolume*>& blendVols, LocalVariablesContainer& aLVC, int mode) const {
    ...
    ATH_MSG_VERBOSE(" zMin " << zMin << " zMax " << zMax << " rMin " << rMin << " rMax " << rMax << " rMaxc " << rMaxc << " phi limits "
                             << pMin << " phiMax " << pMax << " phiLim " << phiLim);
            ...
            for (unsigned int i = 0; i < (*aLVC.m_stationSpan)[gMode]->size(); i++) {
                ...
                if (rLimit && (((*s)[0] < zMax && (*s)[1] > zMin) || meanZOK)) {
                    ...
                    if (accepted) {
                        ...
                        ATH_MSG_VERBOSE(" active volume accepted by rLimit " << station->name() << " zMin " << zMin << " zMax " << zMax
                                                                             << " rMin " << rMin << " rMax " << rMax << " PhiMin " << pMin
                                                                             << " PhiMax " << pMax);
                    }
                } else {
...
bool Muon::MuonTrackingGeometryBuilder::enclosed(const Trk::Volume* vol, const Muon::Span* s, LocalVariablesContainer& aLVC) const {
    bool encl = false;
    double tol = 1.;
...
void Muon::MuonTrackingGeometryBuilder::checkVolume(Trk::TrackingVolume* vol) {
    ...
    std::cout << "cylinder dimensions:innerR,outerR,halfZ,halfPhi:" << cyl->innerRadius() << "," << cyl->outerRadius() << cyl->halflengthZ()
              << "," << cyl->halfPhiSector() << std::endl;
    ...
    std::cout << "glue volumes:neg,pos,inn,outer" << (glueDescr.glueVolumes(Trk::negativeFaceXY)).size() << ","
              << (glueDescr.glueVolumes(Trk::positiveFaceXY)).size() << "," << (glueDescr.glueVolumes(Trk::tubeInnerCover)).size() << ","
              << (glueDescr.glueVolumes(Trk::tubeOuterCover)).size() << std::endl;
}

void Muon::MuonTrackingGeometryBuilder::getZParts(LocalVariablesContainer& aLVC) const {
...
void Muon::MuonTrackingGeometryBuilder::getPhiParts(int mode, LocalVariablesContainer& aLVC) const {
    if (mode == 0) {  // trivial
        aLVC.m_adjustedPhi.clear();
...
void Muon::MuonTrackingGeometryBuilder::getHParts(LocalVariablesContainer& aLVC) const {
    // hardcode for the moment
...
void Muon::MuonTrackingGeometryBuilder::getShieldParts(LocalVariablesContainer& aLVC) const {
...
#undef double
    ...
    if (comb) { envVol = calculateVolume(comb->first()) + calculateVolume(comb->second()); }
    if (sub) { return -1; }

    return envVol;
}

void Muon::MuonTrackingGeometryBuilder::blendMaterial(LocalVariablesContainer& aLVC) const {
    // loop over map
