...
// constructor
Muon::MuonStationBuilder::MuonStationBuilder(const std::string& t, const std::string& n, const IInterface* p) : AthAlgTool(t, n, p) {
...
std::vector<Trk::DetachedTrackingVolume*>* Muon::MuonStationBuilder::buildDetachedTrackingVolumes(bool blend) {
    std::vector<Trk::DetachedTrackingVolume*> mStations;
    if (!m_muonMgr) {
        ATH_MSG_FATAL("No muon manager is provided");
        return nullptr;
    }
    // retrieve muon station prototypes from GeoModel
    std::vector<std::unique_ptr< Trk::DetachedTrackingVolume>> msTypes = buildDetachedTrackingVolumeTypes(blend);
    ...
    while (!vol.atEnd()) {
        ...
        if (vname.compare(0, 3, "NSW") == 0 || vname.compare(0, 8, "NewSmall") == 0) {
            ...
            for (unsigned int ish = 0; ish < vols.size(); ish++) {
                bool isLargeSector = std::abs(((vols[ish]).second)[0].translation().phi()) < 0.01;
                ...
                if (!rtrd && !dia && !trd && !rdia) {  // translate from GeoModel ( spacer & non-identified stuff )
                    ...
                    if (trObject) {
                        std::unique_ptr<Trk::TrackingVolume> newType = std::make_unique<Trk::TrackingVolume>(
                            *trObject, vols[ish].first.second->material(), nullptr, nullptr, protoName);
                        layer =
                ...
                if (layer) {
                    ...
                    if (isLargeSector)
                        ATH_MSG_DEBUG("new NSW prototype build for Large sector: "
                    ...
                    if (!isLargeSector)
                        ATH_MSG_DEBUG("new NSW prototype build for Small sector: "
                                      << protoName << ", " << vols[ish].second.size() << " x0 "
                                      << mat.fullMaterial(layTransf.translation())->x0() << " thickness "
                                      << (mat.fullMaterial(layTransf.translation()))->thickness());
                } else {
            ...
            Trk::DetachedTrackingVolume* typeL =
            ...
            Trk::DetachedTrackingVolume* typeS =
            ...
            if (typeL) {
                ...
                for (int it = 1; it < 8; ++it) {
                    ...
                    Trk::DetachedTrackingVolume* newStat = typeL->clone("NSWL", ntransf);
                    ATH_MSG_DEBUG("cloned 1 NSWL station:" << newStat->trackingVolume()->center());
                    // no detailed identification of NSW layer representation
                    Trk::ArraySpan<Trk::Layer* const> lays = newStat->trackingVolume()->confinedArbitraryLayers();
                    for (auto *lay : lays) {
                ...
                if (nClones == 16) {  // have to mirror stations as well
                    ...
                    Trk::DetachedTrackingVolume* mtypeL = typeL->clone("NSWL", ntransf);
                    ATH_MSG_DEBUG("cloned 2 NSWL station mtypeL :" << mtypeL->trackingVolume()->center());
                    // recalculate identifier
                    Trk::ArraySpan<Trk::Layer* const> lays = mtypeL->trackingVolume()->confinedArbitraryLayers();
                    for (auto *lay : lays) {
                        int iType = lay->layerType();
                        if (iType != 0) {
                            Identifier id(iType);
                    ...
                    for (unsigned int it = 1; it < 8; ++it) {
                        // clone station from prototype :: CHECK z<0 side, probably turns in wrong direction
                        Amg::Transform3D ntransf(Amg::AngleAxis3D(it * 0.25 * M_PI, Amg::Vector3D(0., 0., 1.)));
                        Trk::DetachedTrackingVolume* newStat = mtypeL->clone("NSWL", ntransf);
            ...
            if (typeS) {
                ...
                for (unsigned int it = 1; it < 8; ++it) {
                    ...
                    Amg::Transform3D ntransf(Amg::AngleAxis3D(it * 0.25 * M_PI, Amg::Vector3D(0., 0., 1.)));
                    Trk::DetachedTrackingVolume* newStat = typeS->clone("NSWS", ntransf);
                ...
                if (nClones == 16) {  // have to mirror stations as well
                    ...
                    Trk::DetachedTrackingVolume* mtypeS = typeS->clone("NSWL", ntransf);
                    ATH_MSG_DEBUG("cloned NSWS station:" << mtypeS->trackingVolume()->center());
                    // recalculate identifiers
                    Trk::ArraySpan<Trk::Layer* const> lays = mtypeS->trackingVolume()->confinedArbitraryLayers();
                    for (auto *lay : lays) {
                        int iType = lay->layerType();
                        if (iType != 0) {
                            Identifier id(iType);
                    ...
                    for (unsigned int it = 1; it < 8; ++it) {
                        // clone station from prototype :: CHECK z<0 side, probably turns in wrong direction
                        Amg::Transform3D ntransf(Amg::AngleAxis3D(it * 0.25 * M_PI, Amg::Vector3D(0., 0., 1.)));
                        Trk::DetachedTrackingVolume* newStat = mtypeS->clone("NSWL", ntransf);
                        ATH_MSG_DEBUG("cloned NSWS station:" << newStat->trackingVolume()->center());
                        // recalculate identifiers
                        Trk::ArraySpan<Trk::Layer* const> lays= newStat->trackingVolume()->confinedArbitraryLayers();
                        for (auto *lay : lays) {
        ...
        if (vname.size() > 7 && vname.substr(vname.size() - 7, 7) == "Station" &&
            ...
            // try to retrieve
            if (!gmStation) { gmStation = m_muonMgr->getMuonStation(vname.substr(0, 4), eta, phi); }
            // assembly ?
            if (!gmStation) {
            ...
            for ( std::unique_ptr<Trk::DetachedTrackingVolume>& msTV : msTypes) {
                ...
                if ((stName.substr(0, 1) == "T" && stName == msTypeName.substr(0, stName.size())) ||
                    ...
                    if (msTV && gmStation) {
                        ...
                        // clone station from prototype
                        Trk::DetachedTrackingVolume* newStat = msTV->clone(vname, transf);
    ...
    std::vector<Trk::DetachedTrackingVolume*>* muonStations =
        new std::vector<Trk::DetachedTrackingVolume*>(std::move(mStations));
 ...
 std::vector<std::unique_ptr<Trk::DetachedTrackingVolume>> Muon::MuonStationBuilder::buildDetachedTrackingVolumeTypes(bool /*blend*/) {
        ...
        while (!vol.atEnd()) {
            ...
            if (vname.size() > 7 && vname.substr(vname.size() - 7, 7) == "Station" &&
                ...
                for (auto & station : stations) {
                    if (station  && name == station->name()) is++;
                }
                if (is == 0) {
                    ...
                    if (name.substr(0, 2) == "CS" || name.substr(0, 1) == "T") {
                        if (m_muonStationTypeBuilder) {
                            if (name.substr(0, 2) == "CS") {
                                ...
                                // create layer representation
                                std::pair<Trk::Layer*, const std::vector<Trk::Layer*>*> layerRepr =
                                ...
                                for (auto & tgc_station : tgc_stations) {
                                    // create layer representation
                                    std::pair<Trk::Layer*, const std::vector<Trk::Layer*>*> layerRepr =
                                        m_muonStationTypeBuilder->createLayerRepresentation(tgc_station);
                                    // create prototype as detached tracking volume
                                    Trk::DetachedTrackingVolume* typeStat =
                                        new Trk::DetachedTrackingVolume(name, tgc_station, layerRepr.first, layerRepr.second);
                                    stations.emplace_back(typeStat);
                                }
                            }
                        }
...
void Muon::MuonStationBuilder::glueComponents(Trk::DetachedTrackingVolume* stat) const {
...
void Muon::MuonStationBuilder::identifyLayers(Trk::DetachedTrackingVolume* station, int eta, int phi) const {
    ATH_MSG_VERBOSE(name() << " identifying layers ");

    const std::string stationName = station->trackingVolume()->volumeName().substr(0,3);
    ...
    if (stationName[0] == 'C') {
        ...
        if (cscRE) {
            ...
            ATH_MSG_DEBUG("cscRE not found:" << stationName << "," << eta << "," << phi);
        }
    }
    ...
    if (stationName[0]  == 'T') {
        ...
        const int stationInt = st - MuonGM::MuonDetectorManager::NTgcStatTypeOff;
        ...
        if (!tgc || !(station->trackingVolume()->inside(tgc->center(), 0.))) {
            ...
            while (phit < 48) {
                ...
                if (tgct && station->trackingVolume()->inside(tgct->center(), 0.)) {
                    ...
                    const int stationId = m_idHelperSvc->tgcIdHelper().stationName(oldId);
                    const int stationEta = m_idHelperSvc->tgcIdHelper().stationEta(oldId);
                    Identifier stId = m_idHelperSvc->tgcIdHelper().elementID(stationId, stationEta, phi);
        ...
        if (tgc) {
            ...
            Trk::TrackingVolume* assocVol = station->trackingVolume()->associatedSubVolume(gp);
            if (!assocVol) ATH_MSG_DEBUG("wrong tgcROE?" << stationName << "," << etaSt << "," << phiSt);
            if (assocVol && assocVol->confinedLayers()) {
                Trk::BinnedArraySpan<Trk::Layer * const> layers = assocVol->confinedLayers()->arrayObjects();
    ...
    if (m_idHelperSvc->hasMDT() && (stationName.substr(0, 1) == "B" || stationName.substr(0, 1) == "E")) {
        ...
        if (station->trackingVolume()->confinedVolumes()) {
            Trk::BinnedArraySpan<Trk::TrackingVolume * const> cVols = station->trackingVolume()->confinedVolumes()->arrayObjects();
            for (auto *cVol : cVols) {
        ...
        for (auto *cVol : cVols) {
            if (cVol->confinedLayers()) {
                ...
                for (unsigned int il = 0; il < cLays.size(); il++) {
                    ...
                    if (id == 1) ATH_MSG_DEBUG(station->name() << "," << cVol->volumeName() << ", unidentified active layer:" << il);
                }
            }
            if (!cVol->confinedArbitraryLayers().empty()) {
                Trk::ArraySpan<Trk::Layer* const> cLays = cVol->confinedArbitraryLayers();
    ...
    if (station->trackingVolume()->confinedLayers()) {
        Trk::BinnedArraySpan<Trk::Layer * const> cLays = station->trackingVolume()->confinedLayers()->arrayObjects();
        for (unsigned int il = 0; il < cLays.size(); il++) {
            Identifier id(cLays[il]->layerType());
...
void Muon::MuonStationBuilder::identifyPrototype(Trk::TrackingVolume* station, int eta, int phi, const Amg::Transform3D& transf) const {
    ...
    if (m_idHelperSvc->hasMDT() && (stationFirstChar == "B" || stationFirstChar == "E")) {
        ...
        for (int multi = 1; multi <= 2; ++multi) {
            ...
            const MuonGM::MdtReadoutElement* multilayer = m_muonMgr->getMdtReadoutElement(ele_id);
            if (!multilayer) {
                continue;
            }
            ...
            for (int layer = 1; layer <= nLayers; ++layer) {
                ...
                Trk::Layer* assocLay = assocVol->associatedLayer(transf.inverse() * gp);
                                            ...
                                            for (auto *layer : layers) {
                                                if (layer->layerType() != 0 &&
                                                    layer->surfaceRepresentation().isOnSurface(
                                                        transf.inverse() * rpc->stripPos(etaId), false, 0.5 * layer->thickness())) {
                                                    const Amg::Vector3D locPos1 =
                                                        layer->surfaceRepresentation().transform().inverse() * transf.inverse() *
                                                        rpc->stripPos(etaId);
        ...
        if (!cv->getNChildVols()) {
            ...
            for (unsigned int is = 0; is < vols.size(); is++) {
                if (cName == volNames[is]) {
                    if (std::abs((transform * transf).translation().perp() - vols[is].second.front().translation().perp()) < 1.) {
                        found = true;
