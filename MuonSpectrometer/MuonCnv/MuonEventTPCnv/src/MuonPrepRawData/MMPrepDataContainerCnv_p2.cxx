...
StatusCode Muon::MMPrepDataContainerCnv_p2::initialize(MsgStream& log) {
...
const MuonGM::MMReadoutElement* Muon::MMPrepDataContainerCnv_p2::getReadOutElement(const Identifier& id ) const {
...
void Muon::MMPrepDataContainerCnv_p2::transToPers(const Muon::MMPrepDataContainer* transCont, Muon::MMPrepDataContainer_p2* persCont,
                                                  MsgStream& log) {
    if (!m_isInitialized) {
        if (this->initialize(log) != StatusCode::SUCCESS) {
            log << MSG::FATAL << "Could not initialize MMPrepDataContainerCnv_p2 " << endmsg;
        }
    }
    ...
    for (unsigned int pcollIndex = 0; it_Coll != it_CollEnd; ++pcollIndex, ++it_Coll) {
        ...
        for (unsigned int i = 0; i < collection.size(); ++i) {
            ...
            MMPrepData_p2 pchan{};                   // persistent version to fill
            const Identifier chan_id = chan->identify();
            chanCnv.transToPers(chan, &pchan, log);  // convert from MMPrepData to MMPrepData_p2
...
void Muon::MMPrepDataContainerCnv_p2::persToTrans(const Muon::MMPrepDataContainer_p2* persCont, Muon::MMPrepDataContainer* transCont,
                                                  MsgStream& log) {
    // The transient model has a container holding collections (= 64 modules in MmIdHelper
    // each consisting of two MicroMegas quadruplets in reality) and the collections hold channels.
    //
    ...
    for (unsigned int pcollIndex = 0; pcollIndex < pCollEnd; ++pcollIndex) {
        ...
        for (; pchanIndex < pchanEnd; ++pchanIndex, ++chanIndex) {
            const MMPrepData_p2* pchan = &(persCont->m_prds[pchanIndex]);
            ...
            if (!detEl) {
                if (log.level() <= MSG::WARNING) {
                    log << MSG::WARNING << "Muon::MMPrepDataContainerCnv_p2::persToTrans: could not get valid det element for PRD with id="
...
Muon::MMPrepDataContainer* Muon::MMPrepDataContainerCnv_p2::createTransient(const Muon::MMPrepDataContainer_p2* persObj, MsgStream& log) {
    if (!m_isInitialized) {
        if (this->initialize(log) != StatusCode::SUCCESS) {
            log << MSG::FATAL << "Could not initialize MMPrepDataContainerCnv_p2 " << endmsg;
            return nullptr;
