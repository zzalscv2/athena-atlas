...
StatusCode Muon::sTgcPrepDataContainerCnv_p2::initialize(MsgStream &log) {
...
const MuonGM::sTgcReadoutElement* Muon::sTgcPrepDataContainerCnv_p2::getReadOutElement(const Identifier& id ) const {
...
void Muon::sTgcPrepDataContainerCnv_p2::transToPers(const Muon::sTgcPrepDataContainer* transCont,  Muon::sTgcPrepDataContainer_p2* persCont, MsgStream &log) 
{
  
  if(!m_isInitialized) {
    if (this->initialize(log) != StatusCode::SUCCESS) {
      log << MSG::FATAL << "Could not initialize sTgcPrepDataContainerCnv_p2 " << endmsg;
    } 
  }
  ...
  for (pcollIndex = 0; it_Coll != it_CollEnd; ++pcollIndex, ++it_Coll)  {
    ...
    for (unsigned int i = 0; i < collection.size(); ++i) {
      ...
      sTgcPrepData_p2*   pchan = &(persCont->m_prds[pchanIndex]); // persistent version to fill

      chanCnv.transToPers(chan, pchan, log); // convert from sTgcPrepData to sTgcPrepData_p2
...
void  Muon::sTgcPrepDataContainerCnv_p2::persToTrans(const Muon::sTgcPrepDataContainer_p2* persCont, Muon::sTgcPrepDataContainer* transCont, MsgStream &log) 
{
  ...
  for (unsigned int pcollIndex = 0; pcollIndex < pCollEnd; ++pcollIndex) {
    ...
    for (; pchanIndex < pchanEnd; ++ pchanIndex, ++chanIndex) {
      const sTgcPrepData_p2* pchan = &(persCont->m_prds[pchanIndex]);
	...
	if (result&&log.level() <= MSG::WARNING) 
	  log << MSG::WARNING<< " Muon::sTgcPrepDataContainerCnv_p2::persToTrans: problem converting Identifier to DE hash "<<endmsg;
	const MuonGM::sTgcReadoutElement* detEl =
	  getReadOutElement(clusId); 
	if (!detEl) {
	  log << MSG::WARNING<< "Muon::sTgcPrepDataContainerCnv_p2::persToTrans: could not get valid det element for PRD with id="<<clusId<<". Skipping."<<endmsg;
	  continue;
	}
...
//================================================================
Muon::sTgcPrepDataContainer* Muon::sTgcPrepDataContainerCnv_p2::createTransient(const Muon::sTgcPrepDataContainer_p2* persObj, MsgStream& log) 
{
    if(!m_isInitialized) {
        if (this->initialize(log) != StatusCode::SUCCESS) {
            log << MSG::FATAL << "Could not initialize sTgcPrepDataContainerCnv_p2 " << endmsg;
