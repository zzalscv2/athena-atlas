...
StatusCode Muon::sTgcPrepDataContainerCnv_p3::initialize(MsgStream &log) {
...
const MuonGM::sTgcReadoutElement* Muon::sTgcPrepDataContainerCnv_p3::getReadOutElement(const Identifier& id ) const {
...
void Muon::sTgcPrepDataContainerCnv_p3::transToPers(const Muon::sTgcPrepDataContainer* transCont,  Muon::sTgcPrepDataContainer_p3* persCont, MsgStream &log) 
{
  
  if(!m_isInitialized) {
    if (this->initialize(log) != StatusCode::SUCCESS) {
      log << MSG::FATAL << "Could not initialize sTgcPrepDataContainerCnv_p3 " << endmsg;
    } 
  }
  ...
  for (pcollIndex = 0; it_Coll != it_CollEnd; ++pcollIndex, ++it_Coll)  {
    ...
    for (unsigned int i = 0; i < collection.size(); ++i) {
      ...
      sTgcPrepData_p3*   pchan = &(persCont->m_prds[pchanIndex]); // persistent version to fill

      chanCnv.transToPers(chan, pchan, log); // convert from sTgcPrepData to sTgcPrepData_p3
...
void  Muon::sTgcPrepDataContainerCnv_p3::persToTrans(const Muon::sTgcPrepDataContainer_p3* persCont, Muon::sTgcPrepDataContainer* transCont, MsgStream &log) 
{
  ...
  for (unsigned int pcollIndex = 0; pcollIndex < pCollEnd; ++pcollIndex) {
    ...
    for (; pchanIndex < pchanEnd; ++ pchanIndex, ++chanIndex) {
      const sTgcPrepData_p3* pchan = &(persCont->m_prds[pchanIndex]);
	...
	if (result&&log.level() <= MSG::WARNING) 
	  log << MSG::WARNING<< " Muon::sTgcPrepDataContainerCnv_p3::persToTrans: problem converting Identifier to DE hash "<<endmsg;
	const MuonGM::sTgcReadoutElement* detEl =
	  getReadOutElement(clusId); 
	if (!detEl) {
	  log << MSG::WARNING<< "Muon::sTgcPrepDataContainerCnv_p3::persToTrans: could not get valid det element for PRD with id="<<clusId<<". Skipping."<<endmsg;
	  continue;
	}
...
//================================================================
Muon::sTgcPrepDataContainer* Muon::sTgcPrepDataContainerCnv_p3::createTransient(const Muon::sTgcPrepDataContainer_p3* persObj, MsgStream& log) 
{
    if(!m_isInitialized) {
        if (this->initialize(log) != StatusCode::SUCCESS) {
            log << MSG::FATAL << "Could not initialize sTgcPrepDataContainerCnv_p3 " << endmsg;
