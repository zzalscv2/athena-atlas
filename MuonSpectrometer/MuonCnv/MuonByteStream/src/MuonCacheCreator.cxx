...
MuonCacheCreator::MuonCacheCreator(const std::string &name, ISvcLocator *pSvcLocator) :
    IDCCacheCreatorBase(name, pSvcLocator), m_MdtCsmCacheKey(""), m_CscCacheKey(""), m_RpcCacheKey(""), m_TgcCacheKey("") {
...
StatusCode MuonCacheCreator::initialize() {
    ...
    if (!m_CscCacheKey.key().empty() && !m_idHelperSvc->hasCSC()) {
        ATH_MSG_WARNING("CscCacheKey is non-empty (" << m_CscCacheKey.key()
...
StatusCode MuonCacheCreator::execute(const EventContext &ctx) const {
    ...
    int maxHashMDTs = m_idHelperSvc->mdtIdHelper().stationNameIndex("BME") != -1 ? m_idHelperSvc->mdtIdHelper().detectorElement_hash_max()
                                                                                 : m_idHelperSvc->mdtIdHelper().module_hash_max();
    if (!m_MdtCsmCacheKey.key().empty()) { ATH_CHECK(createContainer<false>(m_MdtCsmCacheKey, maxHashMDTs, ctx)); }
