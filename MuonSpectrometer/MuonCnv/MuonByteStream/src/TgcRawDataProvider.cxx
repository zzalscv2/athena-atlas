...
Muon::TgcRawDataProvider::TgcRawDataProvider(const std::string& name, ISvcLocator* pSvcLocator) :
...
StatusCode Muon::TgcRawDataProvider::execute(const EventContext& ctx) const {
    ...
    if (m_seededDecoding) {
        ...
        for (auto roi : *muonRoI) {
            ...
            // decode the ROBs
            if (m_rawDataTool->convert(tgc_hash_ids, ctx).isFailure()) { ATH_MSG_ERROR("RoI seeded BS conversion into RDOs failed"); }
