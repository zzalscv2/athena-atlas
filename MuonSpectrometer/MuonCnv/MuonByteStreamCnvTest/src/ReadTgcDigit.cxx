...
ReadTgcDigit::ReadTgcDigit(const std::string& name, ISvcLocator* pSvcLocator) : AthAlgorithm(name, pSvcLocator), m_ntuplePtr(nullptr) {
...
StatusCode ReadTgcDigit::execute() {
    ...
    for (TgcDigitContainer::const_iterator c = tgc_container->begin(); c != tgc_container->end(); ++c) {
        ...
        for (TgcDigitCollection::const_iterator dig = (*c)->begin(); dig != (*c)->end(); ++dig) {
            ...
            ATH_MSG_DEBUG(MSG::hex << " N_" << m_idHelperSvc->tgcIdHelper().stationName(id) << " E_"
                                   << m_idHelperSvc->tgcIdHelper().stationEta(id) << " P_" << m_idHelperSvc->tgcIdHelper().stationPhi(id)
                                   << " G_" << m_idHelperSvc->tgcIdHelper().gasGap(id) << " C_"
                                   << m_idHelperSvc->tgcIdHelper().channel(id));
