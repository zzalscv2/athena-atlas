...
ReadRpcDigit::ReadRpcDigit(const std::string &name, ISvcLocator *pSvcLocator) : AthAlgorithm(name, pSvcLocator), m_ntuplePtr(nullptr) {
    ...
    for (RpcDigitContainer::const_iterator c = rpc_container->begin(); c != rpc_container->end(); ++c) {
        ...
        if ((*c)->size() > 0) {
            for (RpcDigitCollection::const_iterator dig = (*c)->begin(); dig != (*c)->end(); ++dig) {
                ...
                if (m_nDig > maxDig - 1) {
                    ATH_MSG_WARNING("Maximum number of RpcDigit in the ntuple reached: " << maxDig);
                    return StatusCode::SUCCESS;
                }
            }
            ...
            if (m_nColl > maxColl - 1) {
                ATH_MSG_WARNING("Maximum number of RpcDigitCollection in the ntuple reached: " << maxColl);
                return StatusCode::SUCCESS;
            }
        }
    }
