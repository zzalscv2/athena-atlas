...
ReadMdtDigit::ReadMdtDigit(const std::string& name, ISvcLocator* pSvcLocator) : AthAlgorithm(name, pSvcLocator), m_ntuplePtr(nullptr) {
    ...
    for (MdtDigitContainer::const_iterator c = mdt_container->begin(); c != mdt_container->end(); ++c) {
        ...
        if ((*c)->size() > 0) {
            for (MdtDigitCollection::const_iterator dig = (*c)->begin(); dig != (*c)->end(); ++dig) {
                ...
                if (m_nDig > maxDig - 1) {
                    ATH_MSG_WARNING("Maximum number of MdtDigit in the ntuple reached: " << maxDig);
                    return StatusCode::SUCCESS;
                }
            }
            ++m_nColl;
            ...
            if (m_nColl > maxColl - 1) {
                ATH_MSG_WARNING("Maximum number of MdtDigitCollection in the ntuple reached: " << maxColl);
                return StatusCode::SUCCESS;
            }
        }
    }
