...
ReadMdtPRD::ReadMdtPRD(const std::string& name, ISvcLocator* pSvcLocator) : AthAlgorithm(name, pSvcLocator), m_ntuplePtr(nullptr) {
        ...
        for (; mdtCollection != lastColl; ++mdtCollection) {
            ...
            if (mdtCollection->size() > 0) {
                ...
                if (m_nColl > maxColl - 1) {
                    ATH_MSG_WARNING("Maximum number of MdtPrepDataCollection in the ntuple reached: " << maxColl);
                    return StatusCode::SUCCESS;
                }
            }
        }
