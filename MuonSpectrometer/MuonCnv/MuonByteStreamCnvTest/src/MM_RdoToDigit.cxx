...
StatusCode MM_RdoToDigit::execute(const EventContext& ctx) const {
    ...
    for (const Muon::MM_RawDataCollection* coll : *rdoContainer) { ATH_CHECK(this->decodeMM(coll, wh_mmDigit.ptr(), collection, oldId)); }

    return StatusCode::SUCCESS;
}

StatusCode MM_RdoToDigit::decodeMM(const Muon::MM_RawDataCollection* rdoColl, MmDigitContainer* mmContainer, MmDigitCollection*& collection,
            ...
            if (oldId != elementId) {
                ...
                if (nullptr == coll) {
                    ...
                    if (mmContainer->addCollection(newCollection, coll_hash).isFailure())
                        ATH_MSG_WARNING("Couldn't record MmDigitCollection with key=" << coll_hash << " in StoreGate!");
                } else {
