...
ReadMdtRDO::ReadMdtRDO(const std::string& name, ISvcLocator* pSvcLocator) : AthAlgorithm(name, pSvcLocator), m_ntuplePtr(nullptr) {
...
StatusCode ReadMdtRDO::execute() {
    ...
    for (; it1 != it2; ++it1) {
        ...
        if ((*it1)->size() > 0) {
            ...
            ATH_MSG_DEBUG("ncsm : " << m_nCsm << " Subdet : " << csm->SubDetId() << " Mrodid : " << csm->MrodId()
                                    << " Csmid : " << csm->CsmId());
            ATH_MSG_DEBUG("Subdet ntuple: " << m_subId[m_nCsm] << " Mrodid ntuple: " << m_rodId[m_nCsm]
            ...
            for (; it3 != it4; ++it3) {
                ...
                if (m_nAmt > maxAmt - 1) {
                    ATH_MSG_WARNING("Maximum number of AmtHit in the ntuple reached: " << maxAmt);
                    return StatusCode::SUCCESS;
                }
            }
            m_nCsm++;
            if (m_nCsm > maxCsm - 1) {
                ATH_MSG_WARNING("Maximum number of MdtCsm in the ntuple reached: " << maxCsm);
                return StatusCode::SUCCESS;
            }
        }
    }
