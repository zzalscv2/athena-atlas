...
StatusCode RpcRdoToRpcDigit::execute(const EventContext& ctx) const {
    ...
    for (; rpcPAD != rdoContainer->end(); ++rpcPAD) {
        if ((*rpcPAD)->size() > 0) { ATH_CHECK(this->decodeRpc(*rpcPAD, wh_rpcDigit.ptr(), collection, rpcCabling)); }
    }

    return StatusCode::SUCCESS;
}

StatusCode RpcRdoToRpcDigit::decodeRpc(const RpcPad* rdoColl, RpcDigitContainer* rpcContainer, RpcDigitCollection*& collection,
    ...
    for (const RpcCoinMatrix* coinMatrix : *rdoColl) {
        ...
        for (const RpcFiredChannel* rpcChan : *coinMatrix) {
            const std::vector<RpcDigit*>* digitVec = m_rpcRdoDecoderTool->getDigit(rpcChan, sectorId, padId, cmaId, rpcCab);
            ...
            for (RpcDigit* newDigit : *digitVec) {
                ...
                if (nullptr == coll) {
                    ...
                    if (rpcContainer->addCollection(newCollection, coll_hash).isFailure())
                        ATH_MSG_WARNING("Couldn't record RpcDigitCollection with key=" << coll_hash << " in StoreGate!");
                } else {
