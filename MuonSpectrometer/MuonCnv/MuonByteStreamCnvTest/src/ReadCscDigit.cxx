...
ReadCscDigit::ReadCscDigit(const std::string& name, ISvcLocator* pSvcLocator) : AthAlgorithm(name, pSvcLocator), m_ntuplePtr(nullptr) {
...
StatusCode ReadCscDigit::execute() {
    ...
    for (CscDigitContainer::const_iterator c = csc_container->begin(); c != csc_container->end(); ++c) {
        if ((*c)->size() > 0) {
            ...
            // loop over digits
            for (CscDigitCollection::const_iterator dig = (*c)->begin(); dig != (*c)->end(); ++dig) {
                ...
                ATH_MSG_DEBUG("Digit = " << m_idHelperSvc->cscIdHelper().show_to_string(id) << " charge = " << m_charge[m_nDig]);
                ++m_nDig;
            }
            ++m_nColl;
        }
