...
StatusCode MuonRdoToMuonDigitTool::decodeMdt(const MdtCsm& rdoColl, MdtDigitMap_t& mdtDigitMap) const {
    ...
    if (!rdoColl.empty()) {
        ...
        for (const MdtAmtHit* amtHit : rdoColl) {
            ...
            if (!coll) { coll = std::make_unique<MdtDigitCollection>(elementId, coll_hash); }
...
StatusCode MuonRdoToMuonDigitTool::decodeCsc(const CscRawDataCollection& rdoColl, CscDigitMap_t& cscDigitMap) const {
    ...
    for (const CscRawData* data : rdoColl) {
        ...
        for (int j = 0; j < width; ++j) {
            ...
            for (uint16_t i = 0; i < samples.size(); ++i) { ATH_MSG_DEBUG("CSC RDO->Digit: " << samples[i]); }
            IdentifierHash coll_hash;
            ...
            if (!coll) { coll = std::make_unique<CscDigitCollection>(stationId, coll_hash); }
            coll->push_back(std::move(newDigit));
        }
    }
    return StatusCode::SUCCESS;
...
StatusCode MuonRdoToMuonDigitTool::decodeRpc(const RpcPad& rdoColl, RpcDigitMap_t& rpcDigitMap, const RpcCablingCondData* rpcCab) const {
    ...
    Identifier elementId = m_idHelperSvc->rpcIdHelper().elementID(stationName, stationEta, stationPhi, doubletR);
...
StatusCode MuonRdoToMuonDigitTool::decodeTgc(const TgcRdo& rdoColl, TgcDigitMap_t& tgcDigitMap) const {
    if (!m_tgcCabling) return StatusCode::FAILURE;
    ...
    for (; itD != itDe; ++itD) {
        ...
        for (int iOr = 0; iOr < 2; ++iOr) {
            ...
            if (iOr != 0) {
                bool o_found = m_tgcCabling->isOredChannel((*itD)->subDetectorId(), (*itD)->rodId(), (*itD)->sswId(), (*itD)->slbId(),
                                                           (*itD)->bitpos());
                // set OR flag
                if (o_found)
                    orFlag = true;
            ...
            bool e_found = m_tgcCabling->getElementIDfromReadoutID(elementId, (*itD)->subDetectorId(), (*itD)->rodId(), (*itD)->sswId(),
                                                                   (*itD)->slbId(), (*itD)->bitpos(), orFlag);
            ...
            if (!e_found) {
                ...
                /* One invalid channel in TGC sector A09:
                   ...
                   bug #48828 */
                if ((*itD)->subDetectorId() == 103 && (*itD)->rodId() == 9 && (*itD)->sswId() == 6 && (*itD)->slbId() == 20 &&
                    (*itD)->bitpos() == 151) {
                ...
                if (show_warning_level || msgLvl(MSG::DEBUG)) {
                    ATH_MSG_WARNING("ElementID not found for "
                                    << " sub=" << (*itD)->subDetectorId() << " rod=" << (*itD)->rodId() << " ssw=" << (*itD)->sswId()
                                    << " slb=" << (*itD)->slbId() << " bitpos=" << (*itD)->bitpos() << " orFlag=" << orFlag);
                }
                continue;
            }
            ...
            if (!coll) { coll = std::make_unique<TgcDigitCollection>(elementId, coll_hash); }
...
StatusCode MuonRdoToMuonDigitTool::decodeSTGC(const Muon::STGC_RawDataCollection& rdoColl, sTgcDigitMap_t& stgcDigitMap) const {
    if (!rdoColl.empty()) {
        ATH_MSG_DEBUG(" Number of RawData in this rdo " << rdoColl.size());
        ...
        for (const Muon::STGC_RawData* data : rdoColl) {
            ...
            if (!coll) { coll = std::make_unique<sTgcDigitCollection>(elementId, coll_hash); }
            coll->push_back(std::move(newDigit));
        }
    }
    return StatusCode::SUCCESS;
