...
CscRdoToCscDigit::CscRdoToCscDigit(const std::string& name, ISvcLocator* pSvcLocator) : AthReentrantAlgorithm(name, pSvcLocator) {}
...
StatusCode CscRdoToCscDigit::execute(const EventContext& ctx) const {
    ...
    for (; it_rdoColl != lastRdoColl; ++it_rdoColl) { ATH_CHECK(this->decodeCsc(*it_rdoColl, wh_cscDigit.ptr(), collection, oldId)); }

    return StatusCode::SUCCESS;
}

StatusCode CscRdoToCscDigit::decodeCsc(const CscRawDataCollection* rdoColl, CscDigitContainer* cscContainer,
                                       CscDigitCollection*& collection, Identifier& oldId) const {
            ...
            if (oldId != stationId) {
                ...
                if (nullptr == coll) {
                    ...
                    if (cscContainer->addCollection(newCollection, coll_hash).isFailure()) {
                        ATH_MSG_WARNING("Couldn't record CscDigitCollection with key=" << coll_hash << " in StoreGate!");
                    }
                } else {
