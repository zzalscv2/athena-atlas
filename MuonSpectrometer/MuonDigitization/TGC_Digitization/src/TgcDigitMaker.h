...
class TgcDigitMaker : public AthMessaging {
    ...
    */
    float timeJitter(const Amg::Vector3D&,
                     CLHEP::HepRandomEngine* rndmEngine) const;

    /**
       Determines whether a hit is detected or not.
    */
    bool efficiencyCheck(const TgcSensor sensor,
                         CLHEP::HepRandomEngine* rndmEngine) const;
    bool efficiencyCheck(const std::string& stationName, const int stationEta,
    ...
    double getEnergyThreshold(const std::string& stationName, int stationEta,
                              int stationPhi, int gasGap,
                              const TgcSensor sensor) const;
    void randomCrossTalk(const TgcDigitCrosstalkData* crosstalk,
                         const Identifier elemId, const int gasGap,
                         const TgcSensor sensor, const int channel,
                         const float posInStrip, const float digitTime,
