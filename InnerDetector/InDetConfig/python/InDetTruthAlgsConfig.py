...
def InDetPRD_MultiTruthMakerSiPUCfg(flags, name="InDetPRD_MultiTruthMakerSiPU",
                                    **kwargs):
...
def InDetPRD_MultiTruthMakerTRTCfg(flags, name="InDetTRT_PRD_MultiTruthMaker",
                                   **kwargs):
    acc = ComponentAccumulator()
...
def InDetPRD_MultiTruthMakerTRTPUCfg(flags, name="InDetTRT_PRD_MultiTruthMakerPU",
                                     **kwargs):
...
def ITkPRD_MultiTruthMakerSiCfg(flags, name="ITkPRD_MultiTruthMakerSi",
                                **kwargs):
    acc = ComponentAccumulator()
...
def ITkPRD_MultiTruthMakerSiPUCfg(flags, name="ITkPRD_MultiTruthMakerSiPU",
                                  **kwargs):
...
def InDetDetailedTrackTruthMakerCfg(flags, name='Maker',**kwargs) :
    acc = ComponentAccumulator()
    kwargs.setdefault("TruthNamePixel", 'PRD_MultiTruthPixel' \
                      if flags.Detector.EnablePixel else "")
    kwargs.setdefault("TruthNameSCT", 'PRD_MultiTruthSCT' \
                      if flags.Detector.EnableSCT else "")

    # for cosmics, at the stage of SiPatternRecognition, the TRT truth information is not yet available
    useTRT = flags.Detector.EnableTRT and \
             not( flags.Beam.Type is BeamType.Cosmics and \
                  (kwargs["DetailedTrackTruthName"] == "SiSPSeededTracksDetailedTruth" or \
                   kwargs["DetailedTrackTruthName"] == "ResolvedTracksDetailedTruth") )
    kwargs.setdefault("TruthNameTRT", 'PRD_MultiTruthTRT' if useTRT else "")

    acc.addEventAlgo(CompFactory.InDet.InDetDetailedTrackTruthMaker(
        name = kwargs["DetailedTrackTruthName"]+name, **kwargs))
    return acc

def ITkDetailedTrackTruthMakerCfg(flags, name='Maker',**kwargs) :
    acc = ComponentAccumulator()
    kwargs.setdefault("TruthNamePixel", 'PRD_MultiTruthITkPixel' \
                      if flags.Detector.EnableITkPixel else "")
    kwargs.setdefault("TruthNameSCT", 'PRD_MultiTruthITkStrip' \
                      if flags.Detector.EnableITkStrip else "")
    kwargs.setdefault("TruthNameTRT", "")
