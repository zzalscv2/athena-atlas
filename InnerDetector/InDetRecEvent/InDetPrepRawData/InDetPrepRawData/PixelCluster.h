...
class PixelCluster final : public SiCluster {
 public:
  ...
  PixelCluster(const Identifier& RDOId,
...
inline float PixelCluster::energyLoss() const {
  if (m_totalCharge > 0.)
    return 3.6 * Gaudi::Units::eV * m_totalCharge;
  else if (m_totalToT > 0)
    return -((float)m_totalToT);
  else
    return 0.;
}
...
inline void PixelCluster::packSplitInformation(bool split, float prob1,
                                               float prob2) {
