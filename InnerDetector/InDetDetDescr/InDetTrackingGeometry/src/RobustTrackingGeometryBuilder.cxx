...
(Trk::TrackingVolume*) const
{
   ...
   for ( unsigned int ilb = 0; ilb < m_layerBuilders.size(); ++ilb){
       ...
       // (a) cylinder
       std::unique_ptr<const std::vector<Trk::CylinderLayer*> > cylinderLayers = m_layerBuilders[ilb]->cylindricalLayers();
       // (a)
       ...
       // (b) discs
       std::unique_ptr<const std::vector<Trk::DiscLayer*> > discLayers = m_layerBuilders[ilb]->discLayers();
   ...
   std::unique_ptr<const std::vector<Trk::CylinderLayer*> > beamPipeVecPtr = m_beamPipeBuilder->cylindricalLayers();
