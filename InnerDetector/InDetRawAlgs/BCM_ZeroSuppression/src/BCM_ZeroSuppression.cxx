...
StatusCode BCM_ZeroSuppression::execute(const EventContext& ctx) const {
  // Retrieve BCM RDO
  SG::ReadHandle<BCM_RDO_Container> bcm_rdo(m_bcmContainerName, ctx);
  if( !bcm_rdo.isValid() ) {
  ...
  SG::WriteHandle<BCM_RDO_Container> bcm_compact_DO(m_bcmOutputName, ctx);
