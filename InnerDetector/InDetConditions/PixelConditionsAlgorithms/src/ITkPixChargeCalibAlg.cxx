...
namespace{
  ...
  defaultThresholds(int bec, int layer){
    static const Thresholds t{600, 24, 75, 0}; //value, sigma, noise, inTimeThresh
    static const Thresholds t0{900, 36, 110, 0}; //only different for first layer of barrel
    if (bec==0 and layer==0) return t0;
    return t;
  }
...
StatusCode ITkPixChargeCalibAlg::execute(const EventContext& ctx) const {
  ...
  for (unsigned int moduleHash{}; moduleHash < m_pixelID->wafer_hash_max(); moduleHash++) {
    ...
    pChargeCalibData -> setTotResolutions(moduleHash, std::vector<Resolutions>(numFE, {0.f, 0.f}));
  }
  ...
  for (unsigned int moduleHash{}; moduleHash < m_pixelID->wafer_hash_max(); moduleHash++) {
    ...
    const InDetDD::SiDetectorElement *element = elements->getDetectorElement(wafer_hash);
    const auto & [numFE, technology] = numChipsAndTechnology(element);
    const Thresholds & defaults = defaultThresholds(barrel_ec, layer);
    if (defaults.value > -0.1) {
      for (InDetDD::PixelDiodeType type : diodeTypes) {
        pChargeCalibData -> setThresholds(type, moduleHash, std::vector<Thresholds>(numFE, defaults));
      }
    }
  }
