...
TProfile* PixelResidualHistograms::GetProfile(const int binnumber, bool RMS, bool savebis){
	...
	if(found0 != std::string::npos) Xvar_name_s.erase(Xvar_name_s.begin(), Xvar_name_s.begin()+found0+1);
	size_t found1 = Xvar_name_s.find('_');
	if(found1 != std::string::npos)	Xvar_name_s.erase(Xvar_name_s.begin()+found1,Xvar_name_s.end());
