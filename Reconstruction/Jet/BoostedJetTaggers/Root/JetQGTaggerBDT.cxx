...
namespace CP {
  ...
  float JetQGTaggerBDT::getScore( const xAOD::Jet& jet ) const {
    ...
    bool validVars = getJetProperties(jet);
  ...
  bool JetQGTaggerBDT::getJetProperties( const xAOD::Jet& jet ) const {
    ...
    if ( m_mode == 1 ) {
      validVars = getPrecomputedVariables(jet);
    }
    else if ( m_mode == 0 ) {
      validVars = calculateVariables(jet);
    }
  ...
  bool JetQGTaggerBDT::getPrecomputedVariables( const xAOD::Jet& jet ) const {
    ...
    if ( !jet.getAttribute<int>("DFCommonJets_QGTagger_NTracks", ntrk) ) {
      ...
      else ATH_MSG_WARNING( "Unable to retrieve DFCommonJets_QGTagger_NTracks" );
    ...
    if ( !jet.getAttribute<float>("DFCommonJets_QGTagger_TracksWidth", trkWidth) ) {
      ...
      else ATH_MSG_WARNING( "Unable to retrieve DFCommonJets_QGTagger_TracksWidth" );
      m_accept.setCutResult("ValidEventContent", false);
      validVars = false;
    }
    if ( !jet.getAttribute<float>("DFCommonJets_QGTagger_TracksC1", trkC1) ) {
      ...
      else ATH_MSG_WARNING( "Unable to retrieve DFCommonJets_QGTagger_TracksC1" );
      m_accept.setCutResult("ValidEventContent", false);
      validVars = false;
    }
  ...
  bool JetQGTaggerBDT::calculateVariables( const xAOD::Jet& jet ) const {
    ...
    if ( evtStore()->retrieve( vxCont, "PrimaryVertices" ).isFailure() ) {
      ...
      else ATH_MSG_WARNING( "Unable to retrieve primary vertex container PrimaryVertices" );
      m_accept.setCutResult("ValidEventContent", false);
      isValid = false;
    }
    else if ( vxCont->empty() ) {
      if ( m_nWarnVar++ < m_nWarnMax ) ATH_MSG_WARNING( "Event has no primary vertices!" );
      else ATH_MSG_WARNING( "Event has no primary vertices!" );
      m_accept.setCutResult("ValidEventContent", false);
      isValid = false;
    }
    else {
    ...
    for ( unsigned i = trackParttmp.size(); i>0; i-- ) {
      ...
      bool accept = (trk->pt()>500 &&
          ...
          (trk->vertex()==primvertex || (!trk->vertex() &&
              std::abs((trk->z0()+trk->vz()-primvertex->z())*sin(trk->theta()))<3.))
      );
      if ( !accept ) {
        trackParttmp.erase(trackParttmp.begin()+i-1);
      }
    ...
    if ( undefTrackWidth ) m_trackwidth = sumPt > 0 ? weightedwidth/sumPt : -0.1;
