...
public:
  virtual void SetUp() {
    ...
    bool ghost{false};
    m_pExtractor_noghost = (makeExtractor(m_toAssociate0, ghost, isTrigger));

    ghost = true;
    m_pExtractor_ghost = (makeExtractor(m_toAssociate1, ghost, isTrigger));
...
TEST_F(PseudoJetContainerTriggerTest, check_fixtures) {
...
TEST_F(PseudoJetContainerTriggerTest, test_noghost) {
...
TEST_F(PseudoJetContainerTriggerTest, test_ghost) {
  // Test assigning IParticles to jets using a single Extractor
...
TEST_F(PseudoJetContainerTriggerTest, test_append) {
