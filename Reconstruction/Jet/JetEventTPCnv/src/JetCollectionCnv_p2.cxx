...
// Implementation file for class JetCollectionCnv_p2
...
static const JetCnv_p2 jetCnv;
static const JetKeyDescriptorCnv_p1 jetkeyCnv;

void 
JetCollectionCnv_p2::persToTrans( const JetCollection_p2* pers, 
                                  JetCollection* trans, 
                                  MsgStream& msg ) const
{
  ...
  trans->reserve( nJets );
  for ( JetCollection_p2::const_iterator 
          itr    = pers->begin(),
          itrEnd = pers->end();
        itr != itrEnd;
        ++itr ) {
...
void 
JetCollectionCnv_p2::transToPers( const JetCollection* trans, 
...
//       << endmsg;
  ...
  JetCollection_p2::iterator    persItr  = pers->begin();
  while(size) {
