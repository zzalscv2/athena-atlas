...
*/
/**
 * @file RecTPCnv/test/MuonCnv_p6_test.cxx
              ...
              const Analysis::Muon& p2)
{
  ...
  assert (p1.outerMatchNumberDoF() == p2.outerMatchNumberDoF());
  assert (p1.segmentChi2OverDoF() == p2.segmentChi2OverDoF());
  compare (*p1.outerMatchChi2(), *p2.outerMatchChi2());
  ...
  assert (p1.statCombinedTrackLink() == p2.statCombinedTrackLink());
  ...
  assert (p1.spectrometerFieldIntegral() == p2.spectrometerFieldIntegral());
  assert (p1.scatteringCurvatureSignificance() == p2.scatteringCurvatureSignificance());
  assert (p1.scatteringNeighbourSignificance() == p2.scatteringNeighbourSignificance());
  assert (p1.momentumBalanceSignificance() == p2.momentumBalanceSignificance());
