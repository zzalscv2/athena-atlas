...
*/
/**
 * @file RecTPCnv/test/MuonCnv_p1_test.cxx
              ...
              const Analysis::Muon& p2)
{
  ...
  //assert (p1.muonSegmentLink() == p2.muonSegmentLink()); // in p2
  ...
  //assert (p1.caloEnergyLink() == p2.caloEnergyLink());  // not tested
  assert (p1.alsoFoundByLowPt() == p2.alsoFoundByLowPt());
  assert (p1.alsoFoundByCaloMuonId() == p2.alsoFoundByCaloMuonId());
  //assert (p1.caloMuonAlsoFoundByMuonReco() == p2.caloMuonAlsoFoundByMuonReco()); // in p2
  //assert (p1.isCorrected() == p2.isCorrected()); // in p3
  ...
  //assert (p1.t0() == p2.t0()); // in p2
  //assert (p1.beta() == p2.beta()); // in p2
}


void transToPers (const Analysis::Muon& trans, Muon_p1& pers, MsgStream& msg)
{  
  ...
  const DepositInCaloCnv_p1    depositCnv;
...
void test1 ATLAS_NOT_THREAD_SAFE ()
{
  ...
  //Athena_test::Leakcheck check; // because converter accesses SG
