...
static const std::string URL = "http://crest-undertow-api.web.cern.ch:80/api-v4.0"; 
...
BOOST_AUTO_TEST_SUITE(CrestFunctionsTest)
  ...
  BOOST_AUTO_TEST_CASE(urlBase_test){
    CrestFunctions cfunctions(URL);
    auto baseUrl=cfunctions.getURLBase();
    BOOST_TEST(baseUrl == "http://crest-undertow-api.web.cern.ch:80/api-v4.0");
  }
  ...
  BOOST_AUTO_TEST_CASE(extractHashFromJson_test){
    ...
    BOOST_TEST(cfunctions.extractHashFromJson(sampleJson) == "dd7a656e41d7b6426164fc411aa7ff51aa4bc047ab885a6e00b976b62291c18b");
    const std::string nonsense{"blubbyPinkBox333"};
    // CrestFunctions cfunctions;
    BOOST_TEST(cfunctions.extractHashFromJson(nonsense) == "");
  }
  ...
  BOOST_AUTO_TEST_CASE(getIovsForTag_test){
    ...
    BOOST_TEST(cfunctions.getLastHashForTag(tag, dontUseRealDatabase) == referenceReply);
  }
  ...
  BOOST_AUTO_TEST_CASE(getPayloadForHash_test){
    ...
    BOOST_TEST(cfunctions.getPayloadForHash(hash, dontUseRealDatabase) == referenceReply);
  }
  ...
  BOOST_AUTO_TEST_CASE(extractChannelListFromJson_test){
    ...
    BOOST_TEST((cfunctions.extractChannelListFromJson(metadata1) == expectedReply));
  }
  ...
  BOOST_AUTO_TEST_CASE(channelListForTag_test){
    ...
    BOOST_TEST((cfunctions.channelListForTag(folderTag, dontUseRealDatabase) == expectedReply));
  }
  ...
  BOOST_AUTO_TEST_CASE(getPayloadForTag_test){
    ...
    const std::string referenceReply{R"delim({"data":{"0":["[DB=B2E3B2B6-B76C-DF11-A505-000423D5ADDA][CNT=CollectionTree(LArTTCell_P/LArTTCellMapAtlas)][CLID=DF8C509C-A91A-40B5-B76C-5B57EEE21EC3][TECH=00000202][OID=00000003-00000000]"]}})delim"};
    BOOST_TEST(cfunctions.getPayloadForTag(tag, dontUseRealDatabase)  == referenceReply);
  }
  ...
  BOOST_AUTO_TEST_CASE(extractDescriptionFromJson_test){
    ...
    BOOST_TEST_MESSAGE(cfunctions.extractDescriptionFromJson(jsonDescr));
    BOOST_TEST(cfunctions.extractDescriptionFromJson(jsonDescr) == expectedAnswer);
  }
  ...
  BOOST_AUTO_TEST_CASE(folderDescriptionForTag_test){
    ...
    BOOST_TEST(cfunctions.folderDescriptionForTag(folderTag, dontUseRealDatabase) == expectedReply);
  }
  ...
  BOOST_AUTO_TEST_CASE(resolveCrestTag_test){
    ...
    BOOST_TEST(cfunctions.resolveCrestTag(globalTag, folderName,"",dontUseRealDatabase) == expectedReply);
  }
