...
DbStatus RNTupleContainer::open(DbDatabase& dbH, const std::string& nam,
  ...
  if (!dbH.isValid() or !info) {
    log << DbPrintLvl::Error << "Cannot open container '" << m_name
        << "', invalid Database handle." << DbPrint::endmsg;
    return Error;
  }
  ...
  try {
    ...
    if (mode & pool::CREATE) {
      ...
      for (auto& dsc : m_fieldDescs) {
        if (dsc.hasAuxStore()) {
          ...
          dsc.auxdyn_reader =
              m_rootDb->getNTupleAuxDynReader(ntupleName, dsc.fieldname);
    ...
    log << DbPrintLvl::Debug << "Opened container " << m_name << " of type "
        << ROOTRNTUPLE_StorageType.storageName() << DbPrint::endmsg;
    m_dbH = dbH;
    m_type = info;
    return Success;
...
DbStatus RNTupleContainer::writeObject(ActionList::value_type& action) {
  if (m_isDirty) {
    ...
    log << DbPrintLvl::Error
        << "Attempt to write to an RNTuple Container twice in the same "
           "transaction! "
        << DbPrint::endmsg;
    m_ioBytes = -1;
    return Error;
  ...
  for (auto& dsc : m_fieldDescs) {
    ...
    switch (dsc.typeID()) {
      ...
      case DbColumn::POINTER:
        ...
        try {
          if (dsc.hasAuxStore()) {
            num_bytes += m_ntupleWriter->writeAuxAttributes(
                dsc.fieldname, dsc.getIOStorePtr(), dsc.rows_written);
          }
        } catch (const std::exception& exc) {
          DbPrint err(m_name);
          err << DbPrintLvl::Error
              << "Dynamic attributes writing error: " << exc.what()
              << DbPrint::endmsg;
    ...
    if (!p.ptr) {
      DbPrint err(m_name);
      err << DbPrintLvl::Error
          << "[RNTupleContainer] Could not write an object of type "
          << dsc.typeName() << DbPrint::endmsg;
      return Error;
    }
    ...
    for (auto& dsc : m_fieldDescs) {
      ...
      switch (dsc.typeID()) {
        case DbColumn::BLOB: {
          ...
          DbPrint err(m_name);
          err << DbPrintLvl::Fatal
              << "[RNTupleContainer] - BLOB reading not implemented yet"
              << DbPrint::endmsg;
          return Error;
        } break;
        case DbColumn::ANY:
      ...
      for (auto val_i = entry->begin(); val_i != entry->end(); ++val_i) {
        if (val_i->GetField()->GetName() == dsc.fieldname) {
          if (p.ptr) {
            ...
            NoDTorWrapper<RValue> rfv(
                std::move(val_i->GetField()->GenerateValue()));
            rfv->Read(evt_id);
            *obj_p = rfv->GetRawPtr();
          }
          break;
...
DbStatus RNTupleContainer::select(DbSelect& sel) {
  if (m_ntupleReader) {
    if (sel.criteria().length() == 0 or sel.criteria() == "*") {
      sel.link().second = -1;
      return Success;
    } else {
      DbPrint log(m_name);
      log << DbPrintLvl::Warning
          << "RNTuple selection not implemented, reading everything"
          << DbPrint::endmsg;
      sel.link().second = -1;
      return Success;
    }
