...
// Implementation file for class McEventCollectionCnv_p7
...
#include "HepMCStatusHelperFunctions.h"
...
McEventCollectionCnv_p7::McEventCollectionCnv_p7() :
  Base_t( ),
  m_isPileup(false),m_hepMCWeightSvc("HepMCWeightSvc","McEventCollectionCnv_p7")
{}
                                           ...
                                           MsgStream& msg )
{
  ...
  for ( std::vector<GenEvent_p7>::const_iterator
          itr = persObj->m_genEvents.begin(),
          itrEnd = persObj->m_genEvents.end();
        itr != itrEnd;
        ++itr ) {
    const GenEvent_p7& persEvt = *itr;
                                           ...
                                           MsgStream& msg )
{
  ...
  for ( McEventCollection::const_iterator itr = transObj->begin();
   ...
   //save the weight names to metadata via the HepMCWeightSvc
    ...
    {
     GenEvent_p7& persEvt = persObj->m_genEvents.back();
   ...
   m_hepMCWeightSvc->setWeightNames( genEvt->m_weights.m_names, ctx ).ignore();
    ...
    persObj->m_genEvents.
      push_back( GenEvent_p7( genEvt->m_signal_process_id,
...
HepMC::GenVertexPtr
McEventCollectionCnv_p7::createGenVertex( const McEventCollection_p7& persEvt,
                                          const GenVertex_p7& persVtx,
                                          ...
                                          ) const
{
  ...
  vtx->set_status(HepMC::old_vertex_status_from_new(persVtx.m_id)); // REVERTED STATUS VALUE TO OLD SCHEME 
  ...
  vtx->m_id      = HepMC::old_vertex_status_from_new(persVtx.m_id); // REVERTED STATUS VALUE TO OLD SCHEME
...
McEventCollectionCnv_p7::createGenParticle( const GenParticle_p7& persPart,
                                            ParticlesMap_t& partToEndVtx, HepMC::DataPool& datapools ,const HepMC::GenVertexPtr& parent, bool add_to_output ) const
{
  ...
  p->set_status(HepMC::old_particle_status_from_new(persPart.m_status)); // REVERTED STATUS VALUE TO OLD SCHEME
  ...
  p->m_status              = HepMC::old_particle_status_from_new(persPart.m_status); // REVERTED STATUS VALUE TO OLD SCHEME
...
#ifdef HEPMC3
void McEventCollectionCnv_p7::writeGenVertex( const HepMC::ConstGenVertexPtr& vtx,
                                              McEventCollection_p7& persEvt )
{
  ...
  const int status = HepMC::new_vertex_status_from_old(vtx->status(), HepMC::barcode(vtx)); // UPDATED STATUS VALUE TO NEW SCHEME
  ...
  GenVertex_p7& persVtx = persEvt.m_genVertices.back();
                                              ...
                                              McEventCollection_p7& persEvt ) const
{
  ...
  persEvt.m_genVertices.push_back(
                                  GenVertex_p7( position.x(),
                                                position.y(),
                                                position.z(),
                                                position.t(),
                                                HepMC::new_vertex_status_from_old(vtx.m_id, vtx.m_barcode), // UPDATED STATUS VALUE TO NEW SCHEME
...
#ifdef HEPMC3
int McEventCollectionCnv_p7::writeGenParticle( const HepMC::ConstGenParticlePtr& p,
                                               McEventCollection_p7& persEvt )
{
  ...
  persEvt.m_genParticles.
    emplace_back( mom.px(),
                               ...
                               mom.m(),
                               p->pdg_id(),
                               HepMC::new_particle_status_from_old(p->status(),HepMC::barcode(p)), // UPDATED STATUS VALUE TO NEW SCHEME
                                               ...
                                               McEventCollection_p7& persEvt ) const
{
  ...
  persEvt.m_genParticles.
    push_back( GenParticle_p7( mom.px(),
                               mom.py(),
                               mom.pz(),
                               mom.m(),
                               p.m_pdg_id,
                               HepMC::new_particle_status_from_old(p.m_status,p.m_barcode), // UPDATED STATUS VALUE TO NEW SCHEME
                               p.m_flow.size(),
