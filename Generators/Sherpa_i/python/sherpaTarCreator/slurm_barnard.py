...
class batchJob(batchJobBase.batchJobBase):
    ...
    def submit(self, dryRun=False):
        cpt = 1
        if self.memMB == 1499 or self.memMB == 2499:
            self.memMB = 4800
        cmd = "sbatch -e "+self.basedir+"/"+self.name+".log -o "+self.basedir+"/"+self.name+".log --time="+str(self.hours)+":00:00 --mem-per-cpu="+str(self.memMB)+" -p barnard --cpus-per-task="+str(cpt)+" -N 1 --ntasks "+str(self.nCores)
