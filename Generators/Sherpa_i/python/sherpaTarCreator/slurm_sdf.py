...
class batchJob(batchJobBase.batchJobBase):
    ...
    def submit(self, dryRun=False):
        ...
        if self.memMB == 1499:
            ...
            self.memMB = 2583
            cpt = 2
        cmd = "sbatch --partition=\"usatlas\" --account=\"usatlas\" --kill-on-invalid-dep=yes -e "+self.basedir+"/"+self.name+".log -o "+self.basedir+"/"+self.name+".log --time=96:00:00 --mem-per-cpu="+str(self.memMB)+" --cpus-per-task="+str(cpt)+" -N 1 --ntasks "+str(self.nCores)
