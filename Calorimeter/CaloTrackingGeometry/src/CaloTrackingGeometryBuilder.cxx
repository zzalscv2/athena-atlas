...
Trk::TrackingGeometry* Calo::CaloTrackingGeometryBuilder::trackingGeometry(Trk::TrackingVolume* innerVol) const
{
  ...
  if (!msCutouts.empty()) {
    ...
    ATH_MSG_VERBOSE(" Calo central cylinder dimensions adjusted using EnvelopeSvc:"<<caloDefaultRadius<<","<< caloDefaultHalflengthZ );
  }
  ...
  // last not needed
  if (bpCutouts.size()>1 && bpCutouts.back().second==bpCutouts[bpCutouts.size()-2].second) bpCutouts.erase(bpCutouts.end()-1);
  // end beam pipe envelope within Calo
  for (unsigned int i=0; i<bpCutouts.size(); i++) ATH_MSG_VERBOSE( "Beam pipe dimensions to be used by Calo:"<< i<<":"<< bpCutouts[i].first<<","<<bpCutouts[i].second );
   ...
   if (nCutVol>0) {
     ...
     for (unsigned int i=1; i<zCutStep.size(); i++) {
       ...
       std::pair<Trk::TrackingVolume*,Trk::TrackingVolume*> cutOutBP =
         createBeamPipeVolumes(bpCutouts, zlow, zup, "CutOuts"+ss.str(), rCutOutBP);
...
void Calo::CaloTrackingGeometryBuilder::registerInLayerIndexCaloSampleMap(
...
Calo::CaloTrackingGeometryBuilder::createBeamPipeVolumes(
