...
////////////////////////////////////////////////////////////////
//                                                            //
//  Implementation of class TH2I_LW                           //
//                                                            //
...
#define FLEXHIST() ((reinterpret_cast<Flex2DHisto<int>*>(m_flexHisto)))
#define CALL(x,y) m_rootHisto ? m_rootHisto-> x : reinterpret_cast<Flex2DHisto<int>*>(m_flexHisto)-> y
...
TH2I_LW * TH2I_LW::create( const char* name, const char* title,
			   ...
			   unsigned nbinsy, const double& ymin, const double& ymax )
{
  ...
  TH2I_LW * h;
  if (rootbackend) {
    h = new TH2I_LW(name,title,nbinsx,xmin,xmax,nbinsy,ymin,ymax,rootbackend);
  } else {
    h = MP_NEW(TH2I_LW)(name,title,nbinsx,xmin,xmax,nbinsy,ymin,ymax,rootbackend);
    h->m_nBytesFromPool = sizeof(TH2I_LW);
  }
  return h;
}
			   ...
			   unsigned nbinsy, const double& ymin, const double& ymax )
{
  ...
  TH2I_LW * h;
  if (rootbackend) {
    h = new TH2I_LW(name,title,nbinsx,xbins,nbinsy,ymin,ymax,rootbackend);
  } else {
    h = MP_NEW(TH2I_LW)(name,title,nbinsx,xbins,nbinsy,ymin,ymax,rootbackend);
    h->m_nBytesFromPool = sizeof(TH2I_LW);
  }
  return h;
}
			   ...
			   unsigned nbinsy, const double* ybins )
{
  ...
  TH2I_LW * h;
  if (rootbackend) {
    h = new TH2I_LW(name,title,nbinsx,xmin,xmax,nbinsy,ybins,rootbackend);
  } else {
    h = MP_NEW(TH2I_LW)(name,title,nbinsx,xmin,xmax,nbinsy,ybins,rootbackend);
    h->m_nBytesFromPool = sizeof(TH2I_LW);
  }
  return h;
}
			   ...
			   unsigned nbinsy, const double* ybins )
{
  ...
  TH2I_LW * h;
  if (rootbackend) {
    h = new TH2I_LW(name,title,nbinsx,xbins,nbinsy,ybins,rootbackend);
  } else {
    h = MP_NEW(TH2I_LW)(name,title,nbinsx,xbins,nbinsy,ybins,rootbackend);
    h->m_nBytesFromPool = sizeof(TH2I_LW);
  }
  return h;
}
...
//____________________________________________________________________
TH2I_LW::TH2I_LW( const char* name, const char* title,
		  unsigned nbinsx, const double& xmin, const double& xmax,
...
{
  if (rootbackend) {
    m_rootHisto = new TH2I(name,title,nbinsx,xmin,xmax,nbinsy,ymin,ymax);
  } else {
    m_flexHisto = Flex2DHisto<int>::create(nbinsx,xmin,xmax,nbinsy,ymin,ymax);
  }
}
...
{
  if (rootbackend) {
    m_rootHisto = new TH2I(name,title,nbinsx,xbins,nbinsy,ymin,ymax);
  } else {
    m_flexHisto = Flex2DHisto<int>::create(nbinsx,xbins,nbinsy,ymin,ymax);
  }
}
...
{
  if (rootbackend) {
    m_rootHisto = new TH2I(name,title,nbinsx,xmin,xmax,nbinsy,ybins);
  } else {
    m_flexHisto = Flex2DHisto<int>::create(nbinsx,xmin,xmax,nbinsy,ybins);
  }
}
...
{
  if (rootbackend) {
    m_rootHisto = new TH2I(name,title,nbinsx,xbins,nbinsy,ybins);
  } else {
    m_flexHisto = Flex2DHisto<int>::create(nbinsx,xbins,nbinsy,ybins);
  }
}
...
//____________________________________________________________________
const TH1* TH2I_LW::getROOTHistBaseNoAlloc() const
{
  return m_rootHisto;
}
TH1* TH2I_LW::getROOTHistBaseNoAlloc()
{
  return m_rootHisto;
}
TH1* TH2I_LW::getROOTHistBase() { return getROOTHist(); }
TH2I* TH2I_LW::getROOTHist()
{
  if (!m_rootHisto) {
    TH2I * h = LWHistRootUtils::createRootHisto<int,TH2I_LW,TH2I,Flex2DHisto<int> >(this,FLEXHIST(),m_ownsRootSumw2);
    if (!apply(h))
      assert(false);
...
void TH2I_LW::clear()
{
  LWREPORT
  LWHist::clear();
  Flex2DHisto<int>::destroy(FLEXHIST());
  m_flexHisto = 0;
}

//____________________________________________________________________
void TH2I_LW::clearKeptROOTHist()
{
...
void TH2I_LW::Reset()
{
  ...
  if (m_flexHisto) {
    ...
    Flex2DHisto<int>::destroy(FLEXHIST());
    if (xbins)
      m_flexHisto = ybins ? Flex2DHisto<int>::create(nbinsx,xbins,nbinsy,ybins)
                          : Flex2DHisto<int>::create(nbinsx,xbins,nbinsy,ymin,ymax);
    else
      m_flexHisto = ybins ? Flex2DHisto<int>::create(nbinsx,xmin,xmax,nbinsy,ybins)
                          : Flex2DHisto<int>::create(nbinsx,xmin,xmax,nbinsy,ymin,ymax);
  }
}

//____________________________________________________________________
void TH2I_LW::SetBins(unsigned nbinsx,double xmin,double xmax,
		      unsigned nbinsy,double ymin,double ymax)
{
  if (GetEntries())
    std::cout<<"TH2I_LW Warning: SetBins(..) called on non-empty histogram."
...
const float * TH2I_LW::getVarBinsX() const
{
  assert (!m_rootHisto);
  return FLEXHIST()->getVarBinsX();
}

const float * TH2I_LW::getVarBinsY() const
{
  assert (!m_rootHisto);
  return FLEXHIST()->getVarBinsY();
}

double TH2I_LW::getXMin() const
{
  if (m_rootHisto)
...
void TH2I_LW::Fill(const double& x,const double& y)
{
  LWCOUNTCALL
  if (m_rootHisto)
...
void TH2I_LW::Fill(const double& x,const double& y, const double& w)
{
  LWCOUNTCALL
  if (m_rootHisto)
...
void TH2I_LW::SetEntries(unsigned n) { CALL(SetEntries(n),setEntries(n)); }

void TH2I_LW::getSums( double& sumW, double& sumW2,
...
void TH2I_LW::setSums( const double& sumW,const double&sumW2,const double& sumWX,const double& sumWX2,
...
double TH2I_LW::actualGetBinCenterX(int bin) const
{
  if (m_rootHisto) {
    assert(false);
    return 0;
...
double TH2I_LW::actualGetBinCenterY(int bin) const
{
  if (m_rootHisto) {
    assert(false);
    return 0;
...
double TH2I_LW::Integral() const
{
  return CALL(Integral(),Integral());
}

void TH2I_LW::resetActiveBinLoop()
{
  if (m_rootHisto) {
    m_rootbackend_fastloopbin = 0;
    return;
...
bool TH2I_LW::getNextActiveBin(unsigned& binx, unsigned& biny, double& content, double& error)
{
  if (m_rootHisto) {
...
void TH2I_LW::GetBinContentAndError(unsigned binx, unsigned biny,double& content, double&error) const
{
  LWCOUNTCALL
  if (m_rootHisto) {
...
void TH2I_LW::SetBinContentAndError(unsigned binx, unsigned biny,const double& content, const double& error)
{
  LWCOUNTCALL
  if (m_rootHisto) {
...
void TH2I_LW::scaleContentsAndErrors( const double& fact )
{
