...
////////////////////////////////////////////////////////////////
//                                                            //
//  Implementation of class TH1D_LW                           //
//                                                            //
...
TH1D_LW * TH1D_LW::create( const char* name, const char* title,
			   unsigned nbinsx, const double& xlow, const double& xup )
{
  bool rootbackend(LWHistControls::hasROOTBackend());
  TH1D_LW * h;
  if (rootbackend) {
    h = new TH1D_LW (name,title,nbinsx,xlow,xup,rootbackend);
  } else {
    h = MP_NEW(TH1D_LW)(name,title,nbinsx,xlow,xup,rootbackend);
    h->m_nBytesFromPool = sizeof(TH1D_LW);
  }
  return h;
}

//____________________________________________________________________
TH1D_LW * TH1D_LW::create( const char* name, const char* title, unsigned nbinsx, const double* xbins )
{
  bool rootbackend(LWHistControls::hasROOTBackend());
  TH1D_LW * h;
  if (rootbackend) {
    h = new TH1D_LW(name,title,nbinsx,xbins,rootbackend);
  } else {
    h = MP_NEW(TH1D_LW)(name,title,nbinsx,xbins,rootbackend);
    h->m_nBytesFromPool = sizeof(TH1D_LW);
  }
  return h;
}
...
//____________________________________________________________________
TH1D_LW * TH1D_LW::create( const char* name, const char* title, unsigned nbinsx, const float* xbins )
{
  bool rootbackend(LWHistControls::hasROOTBackend());
  TH1D_LW * h;
  if (rootbackend) {
...
//____________________________________________________________________
TH1D_LW::TH1D_LW( const char* name, const char* title,
		  unsigned nbins,
		  const double& xmin,
		  const double& xmax,
		  bool rootbackend )
...
{
  if (rootbackend) {
    m_rootHisto = new TH1D(name,title,nbins,xmin,xmax);
  } else {
    m_flexHisto = Flex1DHisto<double>::create(nbins,xmin,xmax);
  }
}
...
{
  if (rootbackend) {
    m_rootHisto = new TH1D(name,title,nbins,xbins);
  } else {
    m_flexHisto = Flex1DHisto<double>::create(nbins,xbins);
  }
}
...
TH1* TH1D_LW::getROOTHistBase() { return getROOTHist(); }
TH1D* TH1D_LW::getROOTHist()
{
  if (!m_rootHisto) {
    TH1D* h = LWHistRootUtils::createRootHisto<double,TH1D_LW,TH1D,Flex1DHisto<double> >(this,FLEXHIST(),m_ownsRootSumw2);
    if (!apply(h))
      assert(false);
...
void TH1D_LW::clear()
{
  LWREPORT
  LWHist::clear();
  Flex1DHisto<double>::destroy(FLEXHIST());
  m_flexHisto = 0;
}

//____________________________________________________________________
void TH1D_LW::clearKeptROOTHist()
{
...
void TH1D_LW::Reset()
{
  ...
  if (m_flexHisto) {
    ...
    Flex1DHisto<double>::destroy(FLEXHIST());
    m_flexHisto = xbins ? Flex1DHisto<double>::create(nbins,xbins) : Flex1DHisto<double>::create(nbins,xmin,xmax);

  }
}

//____________________________________________________________________
void TH1D_LW::SetBins(unsigned nbins,double xmin,double xmax)
{
  if (GetEntries())
    std::cout<<"TH1D_LW Warning: SetBins(..) called on non-empty histogram."
...
const float * TH1D_LW::getVarBins() const
{
  if (m_rootHisto) {
    assert(false);
    return 0;
...
void TH1D_LW::Fill(const double& x)
{
  LWCOUNTCALL
  if (m_rootHisto)
    m_rootHisto->Fill(x);
...
void TH1D_LW::Fill(const double& x, const double& w)
{
  LWCOUNTCALL
  if (m_rootHisto)
...
unsigned TH1D_LW::GetNbinsX() const { return CALL(GetNbinsX(),getNBins()); }
double TH1D_LW::GetBinContent(unsigned bin) const { LWCOUNTCALL;return CALL(GetBinContent(bin),getBinContent(bin)); }
double TH1D_LW::GetBinError(unsigned bin) const { LWCOUNTCALL;return CALL(GetBinError(bin),getBinError(bin)); }
void TH1D_LW::SetBinContent(unsigned bin, const double& c) { LWCOUNTCALL;CALL(SetBinContent(bin,c),setBinContent(bin,c)); }
void TH1D_LW::SetBinError(unsigned bin, const double& e ) { LWCOUNTCALL;CALL(SetBinError(bin,e),setBinError(bin,e)); }
unsigned TH1D_LW::GetEntries() const
{
  if (m_rootHisto)
...
void TH1D_LW::SetEntries(unsigned n) { CALL(SetEntries(n),setEntries(n)); }


void TH1D_LW::getSums( double& sumW, double& sumW2,
...
void TH1D_LW::setSums( const double& sumW,const double&sumW2,const double& sumWX,const double& sumWX2 )
{
  if (m_rootHisto) {
...
double TH1D_LW::actualGetBinCenterX(int bin) const
{
...
unsigned TH1D_LW::actualFindBinX(const double&x) const
{
  assert(!m_rootHisto);//Should have called the TAxis method
  return FLEXHIST()->valueToBin(x);
}

double TH1D_LW::Integral() const { return CALL(Integral(),Integral()); }
void TH1D_LW::resetActiveBinLoop() {
  if (m_rootHisto) {
    m_rootbackend_fastloopbin = 0;
    return;
  }
...
bool TH1D_LW::getNextActiveBin(unsigned& bin, double& content, double& error) {
  if (m_rootHisto) {
...
void TH1D_LW::GetBinContentAndError(unsigned bin,double& content, double&error) const
{
  LWCOUNTCALL
  if (m_rootHisto) {
...
void TH1D_LW::SetBinContentAndError(unsigned bin,const double& content, const double& error)
{
  LWCOUNTCALL
  if (m_rootHisto) {
...
void TH1D_LW::scaleContentsAndErrors( const double& fact )
{
