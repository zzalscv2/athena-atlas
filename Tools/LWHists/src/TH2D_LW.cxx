...
////////////////////////////////////////////////////////////////
//                                                            //
//  Implementation of class TH2D_LW                           //
//                                                            //
...
#define FLEXHIST() ((reinterpret_cast<Flex2DHisto<double>*>(m_flexHisto)))
#define CALL(x,y) m_rootHisto ? m_rootHisto-> x : reinterpret_cast<Flex2DHisto<double>*>(m_flexHisto)-> y
...
TH2D_LW * TH2D_LW::create( const char* name, const char* title,
			   ...
			   unsigned nbinsy, const double& ymin, const double& ymax )
{
  ...
  TH2D_LW * h;
  if (rootbackend) {
    h = new TH2D_LW(name,title,nbinsx,xmin,xmax,nbinsy,ymin,ymax,rootbackend);
  } else {
    h = MP_NEW(TH2D_LW)(name,title,nbinsx,xmin,xmax,nbinsy,ymin,ymax,rootbackend);
    h->m_nBytesFromPool = sizeof(TH2D_LW);
  }
  return h;
}
			   ...
			   unsigned nbinsy, const double& ymin, const double& ymax )
{
  ...
  TH2D_LW * h;
  if (rootbackend) {
    h = new TH2D_LW(name,title,nbinsx,xbins,nbinsy,ymin,ymax,rootbackend);
  } else {
    h = MP_NEW(TH2D_LW)(name,title,nbinsx,xbins,nbinsy,ymin,ymax,rootbackend);
    h->m_nBytesFromPool = sizeof(TH2D_LW);
  }
  return h;
}
			   ...
			   unsigned nbinsy, const double* ybins )
{
  ...
  TH2D_LW * h;
  if (rootbackend) {
    h = new TH2D_LW(name,title,nbinsx,xmin,xmax,nbinsy,ybins,rootbackend);
  } else {
    h = MP_NEW(TH2D_LW)(name,title,nbinsx,xmin,xmax,nbinsy,ybins,rootbackend);
    h->m_nBytesFromPool = sizeof(TH2D_LW);
  }
  return h;
}
			   ...
			   unsigned nbinsy, const double* ybins )
{
  ...
  TH2D_LW * h;
  if (rootbackend) {
    h = new TH2D_LW(name,title,nbinsx,xbins,nbinsy,ybins,rootbackend);
  } else {
    h = MP_NEW(TH2D_LW)(name,title,nbinsx,xbins,nbinsy,ybins,rootbackend);
    h->m_nBytesFromPool = sizeof(TH2D_LW);
  }
  return h;
}
...
//____________________________________________________________________
TH2D_LW::TH2D_LW( const char* name, const char* title,
		  unsigned nbinsx, const double& xmin, const double& xmax,
...
{
  if (rootbackend) {
    m_rootHisto = new TH2D(name,title,nbinsx,xmin,xmax,nbinsy,ymin,ymax);
  } else {
    m_flexHisto = Flex2DHisto<double>::create(nbinsx,xmin,xmax,nbinsy,ymin,ymax);
  }
}
...
{
  if (rootbackend) {
    m_rootHisto = new TH2D(name,title,nbinsx,xbins,nbinsy,ymin,ymax);
  } else {
    m_flexHisto = Flex2DHisto<double>::create(nbinsx,xbins,nbinsy,ymin,ymax);
  }
}
...
{
  if (rootbackend) {
    m_rootHisto = new TH2D(name,title,nbinsx,xmin,xmax,nbinsy,ybins);
  } else {
    m_flexHisto = Flex2DHisto<double>::create(nbinsx,xmin,xmax,nbinsy,ybins);
  }
}
...
{
  if (rootbackend) {
    m_rootHisto = new TH2D(name,title,nbinsx,xbins,nbinsy,ybins);
  } else {
    m_flexHisto = Flex2DHisto<double>::create(nbinsx,xbins,nbinsy,ybins);
  }
}
...
TH1* TH2D_LW::getROOTHistBase() { return getROOTHist(); }
TH2D* TH2D_LW::getROOTHist()
{
  if (!m_rootHisto) {
    TH2D * h = LWHistRootUtils::createRootHisto<double,TH2D_LW,TH2D,Flex2DHisto<double> >(this,FLEXHIST(),m_ownsRootSumw2);
    if (!apply(h))
      assert(false);
...
void TH2D_LW::clear()
{
  LWREPORT
  LWHist::clear();
  Flex2DHisto<double>::destroy(FLEXHIST());
  m_flexHisto = 0;
}
...
void TH2D_LW::Reset()
{
  ...
  if (m_flexHisto) {
    ...
    Flex2DHisto<double>::destroy(FLEXHIST());
    if (xbins)
      m_flexHisto = ybins ? Flex2DHisto<double>::create(nbinsx,xbins,nbinsy,ybins)
                          : Flex2DHisto<double>::create(nbinsx,xbins,nbinsy,ymin,ymax);
    else
      m_flexHisto = ybins ? Flex2DHisto<double>::create(nbinsx,xmin,xmax,nbinsy,ybins)
                          : Flex2DHisto<double>::create(nbinsx,xmin,xmax,nbinsy,ymin,ymax);
  }
}

//____________________________________________________________________
void TH2D_LW::SetBins(unsigned nbinsx,double xmin,double xmax,
		      unsigned nbinsy,double ymin,double ymax)
{
  if (GetEntries())
    std::cout<<"TH2D_LW Warning: SetBins(..) called on non-empty histogram."
...
void TH2D_LW::Fill(const double& x,const double& y)
{
  LWCOUNTCALL
  if (m_rootHisto)
...
void TH2D_LW::Fill(const double& x,const double& y, const double& w)
{
  LWCOUNTCALL
  if (m_rootHisto)
...
double TH2D_LW::GetBinContent(unsigned binx, unsigned biny) const
{
  LWCOUNTCALL;return CALL(GetBinContent(binx,biny),getBinContent(binx,biny));
}

double TH2D_LW::GetBinError(unsigned binx, unsigned biny) const
{
  LWCOUNTCALL;return CALL(GetBinError(binx,biny),getBinError(binx,biny));
}

void TH2D_LW::SetBinContent(unsigned binx, unsigned biny, const double& c)
{
  LWCOUNTCALL;return CALL(SetBinContent(binx,biny,c),setBinContent(binx,biny,c));
}

void TH2D_LW::SetBinError(unsigned binx, unsigned biny, const double& e )
{
...
void TH2D_LW::SetEntries(unsigned n) { CALL(SetEntries(n),setEntries(n)); }

void TH2D_LW::getSums( double& sumW, double& sumW2,
...
void TH2D_LW::setSums( const double& sumW,const double&sumW2,const double& sumWX,const double& sumWX2,
...
bool TH2D_LW::getNextActiveBin(unsigned& binx, unsigned& biny, double& content, double& error)
{
  if (m_rootHisto) {
...
void TH2D_LW::GetBinContentAndError(unsigned binx, unsigned biny,double& content, double&error) const
{
  LWCOUNTCALL
  if (m_rootHisto) {
...
void TH2D_LW::SetBinContentAndError(unsigned binx, unsigned biny,const double& content, const double& error)
{
  LWCOUNTCALL
  if (m_rootHisto) {
