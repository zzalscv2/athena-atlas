...
////////////////////////////////////////////////////////////////
//                                                            //
//  Implementation of class TH1F_LW                           //
//                                                            //
...
TH1F_LW * TH1F_LW::create( const char* name, const char* title,
			   unsigned nbinsx, const double& xlow, const double& xup )
{
  bool rootbackend(LWHistControls::hasROOTBackend());
  TH1F_LW * h;
  if (rootbackend) {
    h = new TH1F_LW (name,title,nbinsx,xlow,xup,rootbackend);
  } else {
    h = MP_NEW(TH1F_LW)(name,title,nbinsx,xlow,xup,rootbackend);
    h->m_nBytesFromPool = sizeof(TH1F_LW);
  }
  return h;
}

//____________________________________________________________________
TH1F_LW * TH1F_LW::create( const char* name, const char* title, unsigned nbinsx, const double* xbins )
{
  bool rootbackend(LWHistControls::hasROOTBackend());
  TH1F_LW * h;
  if (rootbackend) {
    h = new TH1F_LW(name,title,nbinsx,xbins,rootbackend);
  } else {
    h = MP_NEW(TH1F_LW)(name,title,nbinsx,xbins,rootbackend);
    h->m_nBytesFromPool = sizeof(TH1F_LW);
  }
  return h;
}
...
//____________________________________________________________________
TH1F_LW * TH1F_LW::create( const char* name, const char* title, unsigned nbinsx, const float* xbins )
{
  bool rootbackend(LWHistControls::hasROOTBackend());
  TH1F_LW * h;
  if (rootbackend) {
...
//____________________________________________________________________
TH1F_LW::TH1F_LW( const char* name, const char* title,
		  unsigned nbins,
		  const double& xmin,
		  const double& xmax,
		  bool rootbackend )
...
{
  if (rootbackend) {
    m_rootHisto = new TH1F(name,title,nbins,xmin,xmax);
  } else {
    m_flexHisto = Flex1DHisto<float>::create(nbins,xmin,xmax);
  }
}
...
{
  if (rootbackend) {
    m_rootHisto = new TH1F(name,title,nbins,xbins);
  } else {
    m_flexHisto = Flex1DHisto<float>::create(nbins,xbins);
  }
}
...
//____________________________________________________________________
const TH1* TH1F_LW::getROOTHistBaseNoAlloc() const
{
  return m_rootHisto;
}
TH1* TH1F_LW::getROOTHistBaseNoAlloc()
{
  return m_rootHisto;
}
TH1* TH1F_LW::getROOTHistBase() { return getROOTHist(); }
TH1F* TH1F_LW::getROOTHist()
{
  if (!m_rootHisto) {
    TH1F* h = LWHistRootUtils::createRootHisto<float,TH1F_LW,TH1F,Flex1DHisto<float> >(this,FLEXHIST(),m_ownsRootSumw2);
    if (!apply(h))
      assert(false);
...
void TH1F_LW::clear()
{
  LWREPORT
  LWHist::clear();
  Flex1DHisto<float>::destroy(FLEXHIST());
  m_flexHisto = 0;
}

//____________________________________________________________________
void TH1F_LW::clearKeptROOTHist()
{
...
void TH1F_LW::Reset()
{
  ...
  if (m_flexHisto) {
    ...
    Flex1DHisto<float>::destroy(FLEXHIST());
    m_flexHisto = xbins ? Flex1DHisto<float>::create(nbins,xbins) : Flex1DHisto<float>::create(nbins,xmin,xmax);

  }
}

//____________________________________________________________________
void TH1F_LW::SetBins(unsigned nbins,double xmin,double xmax)
{
  if (GetEntries())
    std::cout<<"TH1F_LW Warning: SetBins(..) called on non-empty histogram."
...
void TH1F_LW::Fill(const double& x)
{
  LWCOUNTCALL
  if (m_rootHisto)
    m_rootHisto->Fill(x);
...
void TH1F_LW::Fill(const double& x, const double& w)
{
  LWCOUNTCALL
  if (m_rootHisto)
...
unsigned TH1F_LW::GetNbinsX() const { return CALL(GetNbinsX(),getNBins()); }
double TH1F_LW::GetBinContent(unsigned bin) const { LWCOUNTCALL;return CALL(GetBinContent(bin),getBinContent(bin)); }
double TH1F_LW::GetBinError(unsigned bin) const { LWCOUNTCALL;return CALL(GetBinError(bin),getBinError(bin)); }
void TH1F_LW::SetBinContent(unsigned bin, const double& c) { LWCOUNTCALL;CALL(SetBinContent(bin,c),setBinContent(bin,c)); }
void TH1F_LW::SetBinError(unsigned bin, const double& e ) { LWCOUNTCALL;CALL(SetBinError(bin,e),setBinError(bin,e)); }
unsigned TH1F_LW::GetEntries() const
{
  if (m_rootHisto)
...
void TH1F_LW::SetEntries(unsigned n) { CALL(SetEntries(n),setEntries(n)); }


void TH1F_LW::getSums( double& sumW, double& sumW2,
...
void TH1F_LW::setSums( const double& sumW,const double&sumW2,const double& sumWX,const double& sumWX2 )
{
  if (m_rootHisto) {
...
double TH1F_LW::actualGetBinCenterX(int bin) const
{
...
unsigned TH1F_LW::actualFindBinX(const double&x) const
{
  assert(!m_rootHisto);//Should have called the TAxis method
  return FLEXHIST()->valueToBin(x);
}

double TH1F_LW::Integral() const { return CALL(Integral(),Integral()); }
void TH1F_LW::resetActiveBinLoop() {
  if (m_rootHisto) {
    m_rootbackend_fastloopbin = 0;
    return;
  }
...
bool TH1F_LW::getNextActiveBin(unsigned& bin, double& content, double& error) {
  if (m_rootHisto) {
...
void TH1F_LW::GetBinContentAndError(unsigned bin,double& content, double&error) const
{
  LWCOUNTCALL
  if (m_rootHisto) {
...
void TH1F_LW::SetBinContentAndError(unsigned bin,const double& content, const double& error)
{
  LWCOUNTCALL
  if (m_rootHisto) {
...
void TH1F_LW::scaleContentsAndErrors( const double& fact )
{
