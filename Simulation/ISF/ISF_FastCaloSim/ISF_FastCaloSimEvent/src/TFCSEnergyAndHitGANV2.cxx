...
TFCSEnergyAndHitGANV2::TFCSEnergyAndHitGANV2(const char* name, const char* title):TFCSParametrizationBinnedChain(name,title)
{
  set_GANfreemem();
}
...
const std::string TFCSEnergyAndHitGANV2::get_variable_text(TFCSSimulationState& simulstate,const TFCSTruthState*, const TFCSExtrapolationState*) const
{
  return std::string(Form("layer=%d",simulstate.getAuxInfo<int>("GANlayer"_FCShash)));
}

bool TFCSEnergyAndHitGANV2::fillEnergy(TFCSSimulationState& simulstate, const TFCSTruthState* truth, const TFCSExtrapolationState* extrapol) const
{
    ...
    for (int ix = 1; ix <= xBinNum; ++ix){
      ...
      for (int iy = 1; iy <= binsInAlphaInRBin; ++iy){
        ...
        if (fabs(pdgId) == 22 || fabs(pdgId) == 11) {
          ...
          if (maxHitsInVoxel < 1) maxHitsInVoxel = 1;
          ...
          if (layer != 1 && layer != 5){
            //For layers that are not EMB1 or EMEC1 use a maximum of 10 hits per direction, a higher granularity is needed for the other layers
            int maxNhits = 10;
        ...
        for (int ir = 0; ir < nHitsR; ++ir){
          ...
          for (int ialpha = 1; ialpha <= nHitsAlpha; ++ialpha){
            ...
            if (binsInAlphaInRBin == 1){
              alpha = CLHEP::RandFlat::shoot(simulstate.randomEngine(), -TMath::Pi() , TMath::Pi());
            }
            else {
               alpha = y->GetBinLowEdge(lowEdgeIndex) + y->GetBinWidth(iy)*binsToMerge/(nHitsAlpha+1)*ialpha;
...
FCSReturnCode TFCSEnergyAndHitGANV2::simulate(TFCSSimulationState& simulstate,const TFCSTruthState* truth, const TFCSExtrapolationState* extrapol) const
{
...
void TFCSEnergyAndHitGANV2::unit_test(TFCSSimulationState* simulstate,const TFCSTruthState* truth, const TFCSExtrapolationState* extrapol)
{
  if(!simulstate) {
  ...
  GAN.initializeNetwork(pid,etaMin,"/eos/atlas/atlascerngroupdisk/proj-simul/AF3_Run3/InputsToBigParamFiles/FastCaloGANWeightsVer02");
...
int TFCSEnergyAndHitGANV2::GetAlphaBinsForRBin(const TAxis* x, int ix, int yBinNum) const{
  ...
  if (yBinNum == 32){
    ...
    ATH_MSG_DEBUG("Bin in alpha: "<<binsInAlphaInRBin<< " for r bin: "<<ix<< " (" << x->GetBinLowEdge(ix)<<"-"<< x->GetBinUpEdge(ix)<<")")
;  }
