                  ...
                  if(nodePid == pid){ 
                     if (xmlStrEqual( nodeBin->name, BAD_CAST "Bin" )) {
                        ...
                        if(fabs(etaMid) > nodeEtaMin && fabs(etaMid) < nodeEtaMax){ 
                          ...
                          m_symmetrisedAlpha = ReadBooleanAttribute( "symmetriseAlpha", nodeParticle);
                          ...
                          for( xmlNodePtr nodeLayer = nodeBin->children; nodeLayer != nullptr; nodeLayer = nodeLayer->next ) {
                              if( xmlStrEqual( nodeLayer->name, BAD_CAST "Layer" ) ) {
                                ...
                                else m_relevantlayers.push_back(layer); 
                                double minAlpha = -TMath::Pi();
                                if (m_symmetrisedAlpha && binsInAlpha > 1){
                                  minAlpha = 0;
                                }
                                m_binning[layer] = TH2D(name.c_str(), name.c_str(), xBins, &edges[0], binsInAlpha, minAlpha, TMath::Pi());
                              }
                          }
                        }
                     }
