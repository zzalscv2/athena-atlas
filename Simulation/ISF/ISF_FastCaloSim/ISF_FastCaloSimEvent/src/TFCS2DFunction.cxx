...
void TFCS2DFunction::unit_test ATLAS_NOT_THREAD_SAFE(TH2 *hist,
  ...
  TH1F *hist_pull =
      new TH1F(TString("pull_") + rtof->ClassName(),
               TString("pull for ") + rtof->ClassName(), 80, -4, 4);
  for (int ix = 1; ix <= nbinsx; ++ix) {
    for (int iy = 1; iy <= nbinsy; ++iy) {
...
void TFCS2DFunction::unit_tests
ATLAS_NOT_THREAD_SAFE(TH2 *hist, const char *outfilename, int nrnd) {
  if (hist == nullptr)
