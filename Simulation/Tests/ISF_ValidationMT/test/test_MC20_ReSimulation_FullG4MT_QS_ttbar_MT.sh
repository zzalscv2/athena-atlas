...
then
    ArtPackage=$1
    ArtJobName=$2
    art.py compare grid --entries 10 ${ArtPackage} ${ArtJobName} --mode=semi-detailed --order-trees --diff-root --ignore-leave RecoTimingObj_p1_EVNTtoHITS_timingsOLD --file=resim.CG.HITS.pool.root
    rc4=$?
    status=$rc4
fi
