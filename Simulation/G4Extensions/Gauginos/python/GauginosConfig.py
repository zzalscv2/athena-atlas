...
def getGauginosPhysicsTool(name="GauginosPhysicsTool", **kwargs):
    ...
    if simFlags.specialConfiguration.get_Value().has_key("coannihilationNeutralino"):
        Neutralino = eval(simFlags.specialConfiguration.get_Value().get("coannihilationNeutralino", "0*GeV"))
        kwargs.setdefault("NeutralinoStable",      True);

    elif simFlags.specialConfiguration.get_Value().has_key("GMSBNeutralino"):
        Neutralino = eval(simFlags.specialConfiguration.get_Value().get("GMSBNeutralino", "0*GeV"))
    ...
    kwargs.setdefault("NeutralinoMass",        Neutralino)
