...
def getSleptonsPhysicsTool(name="SleptonsPhysicsTool", **kwargs):
    ...
    if "GMSBStau" in simFlags.specialConfiguration.get_Value() or "coannihilationStau" in simFlags.specialConfiguration.get_Value():
        StauMass = None
        ...
        elif "coannihilationStau" in simFlags.specialConfiguration.get_Value():
...
def getAllSleptonsPhysicsTool(name="AllSleptonsPhysicsTool", **kwargs):
    ...
    if "GMSBStau" in simFlags.specialConfiguration.get_Value() or "coannihilationStau" in simFlags.specialConfiguration.get_Value():
        StauMass = None
        StauLifetime = None
        if "GMSBStau" in simFlags.specialConfiguration.get_Value(): # Check for GMSBStau key word in job options. If found set Stau values.
            StauMass = eval(simFlags.specialConfiguration.get_Value().get("GMSBStau", "None"))
            StauLifetime = eval(simFlags.specialConfiguration.get_Value().get("GMSBStauTime", "None"))
        elif "coannihilationStau" in simFlags.specialConfiguration.get_Value(): # Check for coannihilationStau key word in evgen special configs. This is an option that is normally put in the event gen job options file.
            StauMass = eval(simFlags.specialConfiguration.get_Value().get("coannihilationStau", "None"))
