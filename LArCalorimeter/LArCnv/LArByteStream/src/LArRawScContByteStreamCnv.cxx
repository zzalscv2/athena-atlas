...
const CLID& LArRawScContByteStreamCnv::classID(){
  return ClassID_traits<LArRawSCContainer>::ID() ;
}
...
LArRawScContByteStreamCnv::initialize()
{
  ...
  if (m_ByteStreamEventAccess==NULL)
    {
      (*m_log) <<MSG::ERROR<< "  LArRawScContByteStreamCnv: Can't cast to  ByteStreamCnvSvc " <<endmsg; 
      return StatusCode::FAILURE ;
    }
  ...
  if(StatusCode::SUCCESS != serviceLocator()->getService("ROBDataProviderSvc",svc)){
    ...
    if(m_rdpSvc == 0 ) {
      (*m_log) <<MSG::ERROR<< "  LArRawScContByteStreamCnv: Can't cast to  ByteStreamInputSvc " <<endmsg; 
      return StatusCode::FAILURE;
    }
  }
...
StatusCode
LArRawScContByteStreamCnv::createObj(IOpaqueAddress* pAddr, DataObject*& pObj) 
{//Convert Digits from ByteStream to StoreGate
  ...
  if(key=="SC_ET"){
    et_coll=new LArRawSCContainer();
  }
  else if(key=="SC_ET_ID"){
    et_id_coll=new LArRawSCContainer();
  }
    
  (*m_log) << MSG::DEBUG << "Converting LArRawSCContainer (from ByteStream). key=" << key << endmsg; 
  StatusCode sc;
  ...
  if (sc!=StatusCode::SUCCESS)
   ...
   if(key=="SC_ET"){
    pObj = SG::asStorable(et_coll);
  }
  else if(key=="SC_ET_ID"){
    pObj = SG::asStorable(et_id_coll);
  }
 ...
 StatusCode 
LArRawScContByteStreamCnv::createRep(DataObject* , IOpaqueAddress*& ) 
{
   return StatusCode::SUCCESS;
}
