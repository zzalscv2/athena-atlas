...
namespace Trk
{
  ...
  MaterialAllocator::initialize() {
    ...
    m_messageHelper->setMessage(3,
                                "spectrometerMaterial: did not find MS entrance surface - no MS material taken into account");
    ...
    } else ATH_CHECK(m_trackingGeometryReadKey.initialize());
    ...
    m_calorimeterVolume = &(m_trackingVolumesSvc->volume(ITrackingVolumesSvc::MuonSpectrometerEntryLayer));
    m_indetVolume = &(m_trackingVolumesSvc->volume(ITrackingVolumesSvc::CalorimeterEntryLayer));
      ...
      if ((**m).isScatterer()) {
        if (integrate) {
          ...
          if (++next != measurements.end()
            ...
            if (((**m).intersection(FittedTrajectory).position() - position).mag() <
                1. * Gaudi::Units::mm) integrate = false;
          }
  ...
  MaterialAllocator::leadingSpectrometerTSOS(const TrackParameters& spectrometerParameters,
    ...
    std::unique_ptr<const std::vector<const TrackStateOnSurface*>> extrapolatedTSOS (extrapolatedMaterial(m_extrapolator,
                                                                                           spectrometerParameters,
                                                                                           entranceSurface,
                                                                                           anyDirection,
                                                                                           false,
...
// protect the momentum to avoid excessive Eloss
    ...
    if (parameterVector[Trk::qOverP] == 0.) {
      ...
      if (std::abs(parameterVector[Trk::qOverP]) * Emax <
          1) parameterVector[Trk::qOverP] = trackParameters->charge() / Emax;
    }
...
//  ///////
      ...
      if (preBreak) {
        ...
        if (distance - previousDistance < m_scattererMinGap) offset = 0.5 * (previousDistance - distance);
...
//     printMeasurements(measurements);
  ...
  MaterialAllocator::extrapolatedMaterial(const ToolHandle<IExtrapolator>& extrapolator,
    ...
    for (;
      ...
      double separation = 0.;
      if (TSOS->trackParameters()) separation =
          (TSOS->trackParameters()->position() - (**tg).trackParameters()->position()).mag();
...
//                    if(intersection) std::cout << " iMat 3 ALARM STEP did not intersect! " << std::endl;
    ...
    if (msgLvl(MSG::VERBOSE)) {
      ...
      for (std::vector<const TrackStateOnSurface*>::const_iterator s = indetMaterial->begin();
        ...
        ATH_MSG_VERBOSE(std::setiosflags(std::ios::fixed) << "         material: RZ"
                                                          ...
                                                          << "  deltaE " << std::setw(8) << std::setprecision(4)
                                                          << deltaE
                                                          << " diffP " << std::setw(8) << std::setprecision(
                          4) << p2 - p1);
        p1 = p2;
      }
    }
    ...
    int im = 0;
    ATH_MSG_VERBOSE(
      " measurements.size() " << measurements.size() << " surfaces.size() " << surfaces.size() << " indetMaterial->size() " <<
        indetMaterial->size());
    ...
    for (s = indetMaterial->begin(); s != indetMaterialEnd; ++s) {
      ...
      ATH_MSG_VERBOSE("	(**m).intersection(FittedTrajectory).position() measurement position r " <<
                      (**m).intersection(FittedTrajectory).position().perp() << " z " << (**m).intersection(
                        FittedTrajectory).position().z());
      ATH_MSG_VERBOSE(
        "	(**s).trackParameters()->position() material surface position r "<< (**s).trackParameters()->position().perp() << " z " <<
        (**s).trackParameters()->position().z());
      ...
      while (distance > startDirection.dot((**m).intersection(FittedTrajectory).position() - startPosition)) {
        ...
        if (*m != measurements.back()) {
          ++m;
          ++im;
          ATH_MSG_VERBOSE(" loop im " << im << "	(**m).intersection(FittedTrajectory).position() measurement position r "<<
                          (**m).intersection(
                            FittedTrajectory).position().perp() << " z " << (**m).intersection(
                            FittedTrajectory).position().z());
        } else {
          break;
        }
      }
      ATH_MSG_VERBOSE(" im " << im << " distance measurement " <<
                      startDirection.dot((**m).intersection(FittedTrajectory).position() - startPosition));
      ATH_MSG_VERBOSE(" (**m).intersection(FittedTrajectory).position() measurement position r " <<
                      (**m).intersection(FittedTrajectory).position().perp() << " z " << (**m).intersection(
                        FittedTrajectory).position().z());
...
//           << "  adjacentScatterers " << adjacentScatterers );
  ...
  MaterialAllocator::spectrometerMaterial(std::vector<FitMeasurement*>& measurements,
    ...
    for (; m != measurements.end(); ++m) {
      ...
      double distanceR = std::hypot(positionMst.x() - startPosition.x() , positionMst.y() - startPosition.y());
...
//    if(!m_allowReordering) {
    if (reorderMS && fabs(minDistanceMS) > -2.) ATH_MSG_DEBUG(
        " reorder MS part of track with minimum distance " << minDistanceMS << " minRDistanceMS " << minRDistanceMS << " minZDistanceMS " <<
          minZDistanceMS);
    if (reorderID && fabs(minDistanceID) > -2.) ATH_MSG_DEBUG(
        " reorder ID part of track with minimum distance " << minDistanceID);
//    }
    ...
    const Trk::TrackingVolume* spectrometerEntrance = getSpectrometerEntrance();
    if (!spectrometerEntrance) return;
...
// protect the momentum to avoid excessive Eloss
    ...
    if (parameterVector[Trk::qOverP] == 0.) {
      ...
      if (std::abs(parameterVector[Trk::qOverP]) * Emax <
          1) parameterVector[Trk::qOverP] = endParameters->charge() / Emax;
    }
