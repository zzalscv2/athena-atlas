...
namespace {
  ...
  struct TestTrackState {
    ...
    TestTrackState(rng_t& rng, size_t nMeasurements)
      : surface(Acts::Surface::makeShared<Acts::PlaneSurface>(Vector3::Zero(),
        Vector3::UnitZ())),
  ...
  void fillTrackState(const TestTrackState& pc, TrackStatePropMask mask,
    ...
    //ts.setReferenceSurface(pc.predicted.referenceSurface().getSharedPtr());
          ...
          std::cout<<"pc type "<<typeid(pc.predicted.parameters()).name()<<std::endl;
          std::cout<<"ts type "<<typeid(ts).name()<<std::endl;
    if (ACTS_CHECK_BIT(mask, TrackStatePropMask::Predicted)) {
      std::cout<<" pc predicted "<<pc.predicted.parameters()<<std::endl;
      std::cout<<" ts predicted "<<ts.predicted()<<std::endl;
...
// TODO 
// a test "assert expected "cross-talk" between trackstate proxies" to be added here:
...
// Problem to be solved: TrackState ts  ts.predicted() and ts.parameters() not filled 
