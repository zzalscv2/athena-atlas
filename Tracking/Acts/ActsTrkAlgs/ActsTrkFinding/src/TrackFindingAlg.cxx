...
namespace ActsTrk
{
  ...
  StatusCode TrackFindingAlg::execute(const EventContext &ctx) const
  {
    ...
    {
      uncalibSourceLinks.push_back(m_ATLASConverterTool->UncalibratedMeasurementToSourceLink(*pixelDetEleColl, meas));
    }
    for (auto meas : *stripClusterContainer)
    {
      uncalibSourceLinks.push_back(m_ATLASConverterTool->UncalibratedMeasurementToSourceLink(*stripDetEleColl, meas));
    }
