...
/**
  ...
  should be done, options are alongPre and oppositePre.
...
/**
 ...
 This virtual base class encapsulates the logics to build pre/post/full update
 material for Layer structures, it hosts 5 different MaterialProperties in the
 following order:

  - fullUpdate          (crossing full Layer in both directions)
  - alongPreUpdate      (passing material to sensitive part in direction of the
 normal vector)
  - alongPostUpdate     (passing material after sensitive part in direction of
 the normal vector)
  - oppositePreUpdate   (passing material to sensitive part in opposite
 direction to the normal vector)
  - poopsitePostUpdate  (passing material after sensitive part in opposite
 direction to the normal vector)
...
/** inline return methods for the pre/post factors */
inline double LayerMaterialProperties::alongPreFactor() const {
  return (1. - m_splitFactor);
}

inline double LayerMaterialProperties::alongPostFactor() const {
  return m_splitFactor;
}

inline double LayerMaterialProperties::oppositePreFactor() const {
  return m_splitFactor;
}

inline double LayerMaterialProperties::oppositePostFactor() const {
  return (1. - m_splitFactor);
}
