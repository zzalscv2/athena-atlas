...
namespace Trk {
  ...
  class TrackSelectionProcessorTool final
   ...
   public:
    ...
    // default methods
    TrackSelectionProcessorTool(const std::string &, const std::string &,
                                const IInterface *);
    virtual ~TrackSelectionProcessorTool();
    virtual StatusCode initialize() override;
    virtual StatusCode finalize() override;

    /**Returns a processed TrackCollection from the passed 'tracks' WITHOUT
 copying or refitting the input tracks. The pointers of the tracks in the input
 collection are copied into the output collection which does NOT own the
 pointers. Clients must ensure that the Input Collection is not deleted before
 the output collection else all pointers in the output collection will be
 invalid!

 @param tracks collection of tracks which will have ambiguities resolved. Will
 not be modified.
       @param prdToTrackMap on optional prd-to-track map being filled by the
 processor.

 @return new collections of tracks, with ambiguities resolved.
  The returned track collection is a SG::VIEW_ELEMENTS container, meaning that
 it should NOT be written to disk. Ownership of the collection is passed on
 ...
 The Trk::PrepRawData from each Trk::Track are added to the
 IPRD_AssociationTool*/
    void addNewTracks(TrackScoreMap &trackScoreTrackMap,
