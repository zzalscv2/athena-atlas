...
private:
    ...
    float       m_Rec_theta;   //!< reconstructed track params: angle theta at perigee UNIT:rad
    ...
    // Truth information
    float       m_mc_d0;      //!< matched MC-truth track params: local d<sub>0</sub> at perigee UNIT:mm
    float       m_mc_z0;      //!< matched MC-truth track params: local z<sub>0</sub> at perigee UNIT:mm
    float       m_mc_phi0;    //!< matched MC-truth track params: angle phi at perigee UNIT:rad
    float       m_mc_theta;   //!< matched MC-truth track params: angle theta at perigee UNIT:rad
    float       m_mc_qOverP;  //!< matched MC-truth track params: curvature q/p at perigee UNIT:1/MeV
    float       m_mc_qOverPt; //!< matched MC-truth track params: projected curvature q/pT at perigee UNIT:1/MeV
    float       m_mc_eta;     //!< matched MC-truth track params: pseudorapidity

    float       m_mc_diff_d0; //!< difference reconstructed minus true parameter: d<sub>0</sub> UNIT:mm
    float       m_mc_diff_z0; //!< difference reconstructed minus true parameter: z<sub>0</sub> UNIT:mm
    float       m_mc_diff_phi0; //!< difference reconstructed minus true parameter: angle phi UNIT:rad
    float       m_mc_diff_theta; //!< difference reconstructed minus true parameter: angle theta UNIT:rad
    float       m_mc_diff_qOverP; //!< difference reconstructed minus true parameter: q/p UNIT:1/MeV
    
    float       m_mc_pull_d0;    //!< parameter-pull for d<sub>0</sub> UNIT:1
    float       m_mc_pull_z0;    //!< parameter-pull for z<sub>0</sub> UNIT:1
    float       m_mc_pull_phi0;  //!< parameter-pull for phi           UNIT:1
    float       m_mc_pull_theta; //!< parameter-pull for theta         UNIT:1
    ...
    float       m_mc_prob;       //!< match probability for the truth particle (calculated by the job's configured TrackTruthSelector)
