...
private:
    ...
    std::vector<IValidationNtupleHelperTool*> m_PixelHelperTools;     //!< the helper tools for Pixel data
    std::vector<IValidationNtupleHelperTool*> m_SCTHelperTools;       //!< the helper tools for SCT data
    std::vector<IValidationNtupleHelperTool*> m_TRTHelperTools;       //!< the helper tools for TRT data
    std::vector<IValidationNtupleHelperTool*> m_MDTHelperTools;       //!< the helper tools for MDT data
    std::vector<IValidationNtupleHelperTool*> m_CSCHelperTools;       //!< the helper tools for CSC data
    std::vector<IValidationNtupleHelperTool*> m_RPCHelperTools;       //!< the helper tools for RPC data
    std::vector<IValidationNtupleHelperTool*> m_TGCHelperTools;       //!< the helper tools for TGC data
    std::vector<IValidationNtupleHelperTool*> m_GeneralHelperTools;   //!< the helper tools for data independent from detector technology
    // DNA     ToolHandle< ITrackBreakpointAnalyser >  m_brempointAnalyser;
    bool      m_pullWarning;    //!< warn only ones, if pull cannot be calculated
    bool      m_UpdatorWarning; //!< warn only ones, if unbiased track states can not be calculated
    bool      m_trkParametersWarning; //!< warn once only, if no track parameters exist
    ...
    int                 m_isUnbiased;  //!< flag if track states are unbiased to the related measurements (0:no, 1:yes)
    std::vector<float>* m_residualLocX; //!< residual in Trk::LocX for each measurement on the track, vector of size nHits, value set to -1000 if meas't was outlier UNIT:mm
    std::vector<float>* m_residualLocY; //!< residual in Trk::LocY for each meas't on the track (value is set to -1000 if no second local coord. exists for the meas't), vector of size nHits UNIT:mm
    std::vector<float>* m_pullLocX; //!< pull in Trk::LocX for each measurement on the track, vector of size nHits, value set to -1000 if meas't was outlier UNIT:1
    std::vector<float>* m_pullLocY; //!< pull in Trk::LocY for each measurement on the track (value is set to -1000 if no second local coord. exists for the meas't), vector of size nHits UNIT:1
    std::vector<int>*   m_DetectorType; //!< Detector type of the measurement, enum according to TrkEventPrimitives/TrackStateDefs.h, vector of size nHits
    std::vector<int>*   m_isOutlier; //!< flag if measurement is outlier (1) or not (0), vector of size nHits UNIT:0:no, 1:yes

    int         m_nPixelHits; //!< number of measurements in the Pixels  (without outliers)
    int         m_nSCTHits;   //!< number of measurements in the SCT (without outliers)
    int         m_nTRTHits;   //!< number of measurements in the TRT (without outliers)
    int         m_nMDTHits;   //!< number of measurements in the MDTs (without outliers)
    int         m_nCSCHits;   //!< number of measurements in the CSC (without outliers)
    int         m_nRPCHits;   //!< number of measurements in the RPC (without outliers)
    int         m_nTGCHits;   //!< number of measurements in the TGC (without outliers)

    std::vector<int>*   m_pixelHitIndex; //!< corresp. index for pixel measurement vector
    std::vector<int>*   m_sctHitIndex; //!< corresp. index for SCT measurement vector
    std::vector<int>*   m_trtHitIndex; //!< corresp. index for TRT measurement vector
    std::vector<int>*   m_mdtHitIndex; //!< corresp. index for MDT measurement vector
    std::vector<int>*   m_cscHitIndex; //!< corresp. index for CSC measurement vector
    std::vector<int>*   m_rpcHitIndex; //!< corresp. index for RPC measurement vector
    std::vector<int>*   m_tgcHitIndex; //!< icorresp. index for TGC measurement vector
    
    // hole data:
    int         m_nHoles;                   //!< number of holes on track
    int         m_nPixelHoles;                   //!< number of Pixel holes on track
    int         m_nSctHoles;                   //!< number of SCT holes on track
    std::vector<int>*   m_HoleDetectorType; //!< detector type of the hole (vector of size nHoles)
