...
namespace top {
  ...
  void EventSaverFlatNtuple::initialize(std::shared_ptr<top::TopConfig> config, TFile* file,
	...
	systematicTree->makeOutputVariable(m_ASMweights, "ASM_weight");
      }
	...
	if(m_config->useRCJetSubstructure()) {
	  for (const std::string& name : config->rcJetSubstructureVariables()) {
  ...
  void EventSaverFlatNtuple::setupParticleLevelTreeManager(/*const top::ParticleLevelEvent& plEvent*/) {
    ...
    if (m_config->useRCJets()) {
      ...
      //Adding rc JSS variables into the particle level tree
      if(m_config->useRCJetSubstructure()) {
	for (const std::string& name : m_config->rcJetSubstructureVariables()) {
  ...
  void EventSaverFlatNtuple::calculateEvent(const top::Event& event) {
	...
	for (const std::pair<std::string,std::string>& it : m_config->largeRJetSubstructureVariables()) {
	  m_ljet_substructure[it.first][i] = jetPtr->isAvailable<float>(it.second) ? jetPtr->auxdata<float>(it.second) : -999;
        ...
        for (const std::string& taggerName : m_boostedJetTaggersNames) {
          m_ljet_isTagged[taggerName][i] = jetPtr->isAvailable<char>("isTagged_" + taggerName) ? jetPtr->getAttribute<char>("isTagged_" + taggerName) : 0;
	  m_ljet_taggingPassedRangeCheck[taggerName][i] = jetPtr->isAvailable<char>("passedRangeCheck_" + taggerName) ? jetPtr->getAttribute<char>("passedRangeCheck_" + taggerName) : 0;
        }
