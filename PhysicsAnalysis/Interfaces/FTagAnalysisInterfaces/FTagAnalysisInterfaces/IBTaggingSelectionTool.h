...
/*
  ...
  ****Note for getTaggerWeight and getCutValue****
  the LAST defaulted argument is now 'useCTag', but before AB 21.2.221 it was 'useVetoWP', which has the
...
class IBTaggingSelectionTool : virtual public asg::IAsgTool {
    ...
    virtual const Root::TAccept& accept(double /* jet pt */, double /* jet eta */, double /* b-tag weight */, double /* c-tag weight */ ) const = 0;
    ...
    virtual CP::CorrectionCode getTaggerWeight( double /* dl1pb */, double /* dl1pc  */ , double /* dl1pu  */ , double & weight,bool useCTag=false) const = 0;
