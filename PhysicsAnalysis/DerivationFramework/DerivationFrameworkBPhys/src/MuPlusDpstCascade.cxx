...
namespace DerivationFramework {
    ...
    StatusCode MuPlusDpstCascade::initialize() {
        ...
        if(m_vtx0Daug2MassHypo < 0.) m_vtx0Daug2MassHypo = BPhysPVCascadeTools::getParticleMass(pdt, MC::PIPLUS);
    ...
    std::vector<Trk::VxCascadeInfo*> cascadeinfoContainer;
      ...
      for (Trk::VxCascadeInfo* x : cascadeinfoContainer) {
        ...
        const xAOD::Vertex* MuPiVertex = BPhysPVCascadeTools::FindVertex<2>(MuPiContainer, cascadeVertices[1]);
        ...
        std::vector<const xAOD::Vertex*> MuPiVerticestoLink;
    ...
    StatusCode MuPlusDpstCascade::performSearch(std::vector<Trk::VxCascadeInfo*> *cascadeinfoContainer) const
    {
        ...
        for(auto MuPiItr:selectedMuPiCandidates){
            ...
            for(auto d0Itr : selectedD0Candidates){
              ...
              Trk::VertexID vID2 = m_iVertexFitter->nextVertex(tracksMuPi,massesMuPi,vrtList,*state);
              if (m_constrMuPi) {
                std::vector<Trk::VertexID> cnstV;
                cnstV.clear();
                if ( !m_iVertexFitter->addMassConstraint(vID2,tracksMuPi,cnstV,*state,m_vtx0MassHypo).isSuccess() ) {
